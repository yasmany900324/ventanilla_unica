...
function canAccessProcedureRequest(userId, procedureRequest) {
  if (!procedureRequest) {
    return false;
  }
...
function resolveTaskUiConfig({ activeTask, procedureType }) {
...
function resolveTaskDisplayConfig({ activeTask, procedureType }) {
  const taskDefinitionKey = String(activeTask?.taskDefinitionKey || "").trim();
  if (!taskDefinitionKey) {
    return {
      taskDefinitionKey: "",
...
function buildAvailableActions({ procedureRequest, activeTask, procedureType, actorId }) {
  const actions = [];
  ...
  if (procedureRequest?.camundaError) {
    actions.push({
      ...
      endpoint: `/api/admin/procedures/requests/${encodeURIComponent(procedureRequest.id)}/retry-camunda-sync`,
      method: "POST",
      requiresTaskClaim: false,
      requiredVariables: [],
    });
  }
  if (activeTask?.taskDefinitionKey && !procedureRequest?.taskAssigneeId) {
    actions.push({
      actionKey: "claim_task",
      label: `Reclamar tarea: ${taskDisplayName}`,
      displayLabel: "Reclamar tarea",
      endpoint: `/api/admin/procedures/requests/${encodeURIComponent(procedureRequest.id)}/claim-task`,
      method: "POST",
      requiresTaskClaim: false,
      requiredVariables: [],
    });
  ...
  if (activeTask?.taskDefinitionKey) {
    ...
    actions.push({
      ...
      endpoint: `/api/admin/procedures/requests/${encodeURIComponent(procedureRequest.id)}/complete-task`,
      method: "POST",
...
export async function GET(request, { params }) {
  try {
    ...
    if (!canAccessProcedureRequest(administrator.id, procedureRequest)) {
