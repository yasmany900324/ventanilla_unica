...
export function buildQuestionForStep({
  ...
  if (step === CHATBOT_CURRENT_STEPS.LOCATION) {
    ...
    fallbackText =
      "Si querés, podés adjuntar una foto (opcional). Si no, tocá «Omitir foto».";
  }
...
export function buildIncidentResumeReply(collectedData) {
  const base =
    "Listo para registrar la incidencia. Revisá los datos y tocá «Confirmar y crear incidencia», o editá lo que necesites.";
  ...
  if (collectedData.photoStatus === "provided") {
    ...
    if (display) {
      return `Recibí la imagen «${display}». ${base}`;
    }
    return `Recibí la imagen adjunta. ${base}`;
  }
  if (collectedData.photoStatus === "skipped") {
    return `Continuamos sin foto. ${base}`;
  }
  return base;
}
