...
const CANCELLATION_TEXTS = new Set(["no", "cancelar", "cancela", "cancelo", "salir", "detener"]);
...
function hasProcedureSignals(collectedData) {
...
export function shouldSwitchToIncidentFlow({ text, interpretation, collectedData = null }) {
  if (hasProcedureSignals(collectedData)) {
    return false;
  }
  const normalized = normalizeForLookup(text);
  if (!normalized) {
...
export function shouldActivateIncidentFlow({ interpretation, text, contextEntry, collectedData = null }) {
  if (hasProcedureSignals(collectedData)) {
    return false;
  }
  ...
  return shouldSwitchToIncidentFlow({ text, interpretation, collectedData });
}
...
function buildIncidentProcedureLabel(collectedData) {
  const visibleNameCandidates = [
    collectedData?.procedureName,
    collectedData?.procedureDisplayName,
    collectedData?.catalogItemDisplayName,
    collectedData?.catalogItemLabel,
    collectedData?.catalogItemTitle,
  ];
  for (const candidate of visibleNameCandidates) {
    const normalized = normalizeFieldValue(candidate || "", 160);
    if (normalized) {
      return normalized;
    }
  }
...
export function buildProcedureStartReply(procedureName) {
  const normalizedProcedureName =
    typeof procedureName === "string" ? procedureName.replace(/\s+/g, " ").trim().slice(0, 160) : "";
  if (normalizedProcedureName) {
    return `Entendido. Voy a ayudarte con "${normalizedProcedureName}".`;
  }
  return "Entendido. Voy a ayudarte con este trámite.";
}
