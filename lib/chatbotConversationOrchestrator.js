...
export function buildWhatsAppIncidentSummaryPlainText(collectedData) {
  ...
  const riskLine = risk ? `Riesgo: ${risk}.` : "Riesgo: (sin registrar).";
  const evidenceLine = preview?.photoAttached
    ? "Evidencia: hay una foto adjunta."
    : "Evidencia: sin foto adjunta.";
...
export function buildIncidentConfirmationIntroReply(locale = "es", options = null) {
  ...
  if (options?.channel === "whatsapp" && options?.collectedData) {
    ...
    return `${intro}\n\n${summary}\n\nPara crear la incidencia respondé: «sí», «dale», «de acuerdo» o escribí «confirmar». Si querés cambiar algo, decime «corregir datos».`;
  }
  return intro;
}
...
export function buildIncidentConfirmationGateReply(channel = "web") {
  if (channel === "whatsapp") {
    return "Para crear la incidencia, respondé «sí», «dale» o «confirmar». Si querés cambiar algo, decime «corregir datos» o indicá qué dato corregir (ubicación, descripción, riesgo o foto).";
  }
  return "Para crear la incidencia, confirmá con «Confirmar y crear incidencia» o escribí sí, dale o de acuerdo. Si necesitás cambiar algo, tocá «Corregir datos» o decime qué dato querés corregir.";
}
