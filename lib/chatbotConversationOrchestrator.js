...
export function buildIncidentResumeReply(collectedData) {
  const base =
    "Revisá el resumen. Si coincide con lo que pasó, confirmá con «Confirmar y crear incidencia» o respondé sí, dale o de acuerdo. Si algo no coincide, tocá «Corregir datos» o decime qué querés cambiar.";
  if (!collectedData || typeof collectedData !== "object") {
    return base;
  }
  if (collectedData.photoStatus === "provided") {
    const display = sanitizePhotoDisplayName(collectedData.photoAttachmentOriginalName);
    if (display) {
      return `Guardamos tu foto «${display}». ${base}`;
    }
    return `Guardamos tu foto. ${base}`;
  }
  if (collectedData.photoStatus === "skipped") {
    return `Seguimos sin foto. ${base}`;
  }
  return base;
}
