...
export function buildQuestionForStep({
  ...
  if (step === CHATBOT_CURRENT_STEPS.LOCATION) {
    ...
    fallbackText =
      channel === "whatsapp"
        ? "Si querés, podés enviar una foto como evidencia (es opcional). Si no querés adjuntar imagen, respondé «sin foto» o «omitir foto»."
...
export function buildWhatsAppIncidentSummaryPlainText(collectedData) {
  ...
  const typeLine = preview?.typeLine || "Incidencia general";
  const locationLine = preview?.location || "Sin ubicación informada";
  const risk = preview?.riskRaw || "";
  const riskDisplay = risk ? risk : "(sin registrar)";
  const evidenceDisplay = preview?.photoAttached ? "hay una foto adjunta" : "sin foto adjunta";
  return `Resumen del reporte:\n- Tipo: ${typeLine}\n- Ubicación: ${locationLine}\n- Riesgo: ${riskDisplay}\n- Evidencia: ${evidenceDisplay}`;
}
...
export function buildIncidentConfirmationIntroReply(locale = "es", options = null) {
  ...
  if (options?.channel === "whatsapp" && options?.collectedData) {
    const summary = buildWhatsAppIncidentSummaryPlainText(options.collectedData);
    const closing =
      key === "en"
        ? "If everything looks right, I'll go ahead and register it. Does that work for you?"
        : key === "pt"
          ? "Se estiver tudo certo, sigo com o registro. Pode ser?"
          : "Si está todo correcto, sigo con el registro. ¿Te parece bien?";
    return `${intro}\n\n${summary}\n\n${closing}`;
  }
  return intro;
}
...
export function buildIncidentConfirmationGateReply(channel = "web") {
  if (channel === "whatsapp") {
    return "Perdón, no te seguí con eso. Si el resumen te cierra, decime que sí y sigo con el registro; si algo no encaja, contame qué parte querés retocar y lo revisamos.";
  }
  return "Perdón, no te seguí con eso. Si el resumen refleja bien tu reporte, podés usar el botón principal de abajo o escribirme un sí bien claro; si algo no encaja, decime qué parte habría que cambiar y lo vemos.";
}
