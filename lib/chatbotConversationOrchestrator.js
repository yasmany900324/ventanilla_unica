...
export function isTreeIncidentCollectedData(collectedData) {
  const normalizedSubcategory = normalizeForLookup(collectedData?.subcategory || "");
  if (normalizedSubcategory) {
    return normalizedSubcategory.includes("arbol") || normalizedSubcategory.includes("rama");
  }

  return hasTreeSignals(collectedData?.description || "");
}
...
export function buildQuestionForStep({
  ...
  if (step === CHATBOT_CURRENT_STEPS.LOCATION) {
    if (isTreeSpecific) {
      ...
      fallbackText = lowConfidence
        ? "No me quedo clara la ubicacion. Indicame la direccion o referencia exacta del problema."
        : "Indicame la direccion o referencia exacta del problema.";
    }
...
export function buildIncidentSummary(collectedData) {
  const categoryLabel = isTreeIncidentCollectedData(collectedData)
