...
function mapProcedureRequestRow(row) {
  ...
  return {
    ...
    // Transitional alias: today both concepts point to the same DB column.
    // Future split:
    // - assignedToUserId => expediente owner/responsible officer
    // - taskAssigneeId => active Camunda task assignee
    assignedToUserId: row.task_assignee_id || null,
    assignmentScope,
...
export async function listProcedureRequestsForAdmin({
  ...
  const assignmentWhereClause =
    normalizedScope === "mine" && normalizedRequesterUserId
      ? sql`task_assignee_id = ${normalizedRequesterUserId}`
      : normalizedScope === "unassigned"
        ? sql`task_assignee_id IS NULL`
        : normalizedRequesterUserId
          ? sql`(task_assignee_id IS NULL OR task_assignee_id = ${normalizedRequesterUserId})`
          : sql`task_assignee_id IS NULL`;
  const rows = await sql`
    SELECT
      id,
...
export async function listProcedureRequestsForAssignee({
  ...
  const rows = await sql`
    SELECT
      ...
      updated_at
    FROM chatbot_procedure_requests
    WHERE task_assignee_id = ${normalizedAssignee}
    ORDER BY created_at DESC
    LIMIT ${Number.isInteger(limit) && limit > 0 ? Math.min(limit, 500) : 100};
  `;
  return rows.map((row) => mapProcedureRequestRow(row));
...
export async function resolveFuncionarioAssignmentScopeForProcedureRequest({
  ...
  const [row] = await sql`
    SELECT
      CASE
        WHEN pr.task_assignee_id = ${normalizedFuncionario} THEN ${FUNCIONARIO_ASSIGNMENT_SCOPE.ASSIGNED_TO_ME}
        WHEN pr.task_assignee_id IS NULL
...
export async function listProcedureRequestsForFuncionarioInbox({
  ...
  const rows = await sql`
    SELECT
      ...
      END AS assignment_scope,
      (pr.task_assignee_id = ${normalizedFuncionario}) AS is_assigned_to_me,
      (pr.task_assignee_id IS NULL) AS is_available_to_claim
    FROM chatbot_procedure_requests pr
    WHERE
      pr.task_assignee_id = ${normalizedFuncionario}
      OR (
        pr.task_assignee_id IS NULL
        AND pr.procedure_type_id IS NOT NULL
        AND EXISTS (
          SELECT 1
          FROM procedure_type_assignees pta
...
export async function claimProcedureRequestForFuncionarioInbox({
  ...
  if (current.taskAssigneeId && String(current.taskAssigneeId) === normalizedFuncionario) {
    return {
      ok: true,
      idempotent: true,
      procedureRequest: {
  ...
  const [row] = await sql`
    ...
    SET
      task_assignee_id = ${normalizedFuncionario},
      task_claimed_at = NOW(),
      task_claim_expires_at = NULL,
      updated_at = NOW()
    WHERE pr.id = ${normalizedRequestId}
      AND pr.task_assignee_id IS NULL
      AND pr.procedure_type_id IS NOT NULL
      AND EXISTS (
        SELECT 1
        FROM procedure_type_assignees pta
  ...
  `;
  if (row) {
    return { ok: true, idempotent: false, procedureRequest: mapProcedureRequestRow(row) };
  }

  const latest = await getProcedureRequestById(normalizedRequestId);
  if (latest?.taskAssigneeId && String(latest.taskAssigneeId) === normalizedFuncionario) {
    return {
      ok: true,
      idempotent: true,
      procedureRequest: {
