...
import { ensureProcedureAssignmentsSchema } from "./procedureAssignments";
...
export const PROCEDURE_REQUEST_STATUSES = {
  ...
  /** Instancia Camunda activa pero la API de user-tasks no devolvió tarea mapeable (diagnóstico Operate vs Orchestration REST). */
  CAMUNDA_ACTIVE_TASK_NOT_FOUND: "CAMUNDA_ACTIVE_TASK_NOT_FOUND",
  RESOLVED: "RESOLVED",
  REJECTED: "REJECTED",
  CLOSED: "CLOSED",
  ARCHIVED: "ARCHIVED",
...
/** Valor inicial de `camunda_metadata_json` al crear solicitud (columna NOT NULL). */
export const DEFAULT_CAMUNDA_METADATA_ON_CREATE = { syncStatus: "PENDING" };

function resolveCamundaMetadataJsonForInsert(camundaMetadata) {
  if (camundaMetadata && typeof camundaMetadata === "object" && !Array.isArray(camundaMetadata)) {
    return { ...DEFAULT_CAMUNDA_METADATA_ON_CREATE, ...camundaMetadata };
  }
  return { ...DEFAULT_CAMUNDA_METADATA_ON_CREATE };
}
const DEFAULT_WAITING_CITIZEN_INFO_TIMEOUT_HOURS = 48;
export const FUNCIONARIO_ASSIGNMENT_SCOPE = {
  ASSIGNED_TO_ME: "assigned_to_me",
  AVAILABLE: "available",
};
export const TERMINAL_PROCEDURE_STATUSES = new Set([
...
const TERMINAL_STATUSES = TERMINAL_PROCEDURE_STATUSES;
...
function mapProcedureRequestRow(row) {
  ...
  const assignmentScopeFromRow = normalizeText(row?.assignment_scope, 40).toLowerCase();
  const isAssignedToMeFromRow = row?.is_assigned_to_me === true;
  const isAvailableToClaimFromRow = row?.is_available_to_claim === true;
  const assignmentScope =
    assignmentScopeFromRow === FUNCIONARIO_ASSIGNMENT_SCOPE.ASSIGNED_TO_ME ||
    assignmentScopeFromRow === FUNCIONARIO_ASSIGNMENT_SCOPE.AVAILABLE
      ? assignmentScopeFromRow
      : isAssignedToMeFromRow
        ? FUNCIONARIO_ASSIGNMENT_SCOPE.ASSIGNED_TO_ME
        : isAvailableToClaimFromRow
          ? FUNCIONARIO_ASSIGNMENT_SCOPE.AVAILABLE
          : null;
  ...
  return {
    ...
    assignedToUserId: row.inbox_owner_user_id || null,
    inboxOwnerAssignedAt: row.inbox_owner_assigned_at || null,
    assignmentScope,
    isAssignedToMe: assignmentScope === FUNCIONARIO_ASSIGNMENT_SCOPE.ASSIGNED_TO_ME,
    isAvailableToClaim: assignmentScope === FUNCIONARIO_ASSIGNMENT_SCOPE.AVAILABLE,
...
export async function ensureProcedureRequestSchema() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS chatbot_procedure_requests (
      ...
      inbox_owner_user_id TEXT REFERENCES citizens(id) ON DELETE SET NULL,
      inbox_owner_assigned_at TIMESTAMPTZ,
  ...
  await sql`
    ALTER TABLE chatbot_procedure_requests
    ADD COLUMN IF NOT EXISTS inbox_owner_user_id TEXT;
  `;
  await sql`
    ALTER TABLE chatbot_procedure_requests
    ADD COLUMN IF NOT EXISTS inbox_owner_assigned_at TIMESTAMPTZ;
  `;
  await sql`
    ALTER TABLE chatbot_procedure_requests
  ...
  await sql`
    UPDATE chatbot_procedure_requests
    SET sync_max_retry_count = ${DEFAULT_MAX_SYNC_RETRIES}::int
    WHERE sync_max_retry_count IS NULL OR sync_max_retry_count < 1;
  `;
  await sql.unsafe(
    `ALTER TABLE chatbot_procedure_requests ALTER COLUMN sync_max_retry_count SET DEFAULT ${DEFAULT_MAX_SYNC_RETRIES};`
  );
  await sql`
    ALTER TABLE chatbot_procedure_requests
  ...
  `;
  await sql`
    CREATE INDEX IF NOT EXISTS chatbot_procedure_requests_inbox_owner_idx
    ON chatbot_procedure_requests (inbox_owner_user_id, updated_at DESC);
  `;
...
export async function createProcedureRequest({
  ...
  camundaMetadata = undefined,
}) {
  const sql = ensureDatabase();
  await ensureProcedureRequestSchema();
  const id = randomUUID();
  const requestCode = `TRA-${id.slice(0, 8).toUpperCase()}`;
  const camundaMetadataJson = resolveCamundaMetadataJsonForInsert(camundaMetadata);
...
export async function updateProcedureRequestCamundaData({
  ...
  const normalizedStatus = normalizeProcedureStatus(status) || null;
  const normalizedCamundaError = normalizeText(camundaError, 500) || null;
  const normalizedCamundaMetadata =
    camundaMetadata && typeof camundaMetadata === "object" ? camundaMetadata : null;
  const [row] = await sql`
    UPDATE chatbot_procedure_requests
    SET
      ...
      camunda_process_version = CASE
        ...
        WHEN ${Number.isInteger(camundaProcessVersion) && camundaProcessVersion > 0}
          THEN ${camundaProcessVersion ?? null}::int
        ELSE NULL
      END,
      ...
      task_claimed_at = CASE
        ...
        ELSE ${taskClaimedAt ?? null}::timestamptz
      END,
      task_claim_expires_at = CASE
        ...
        ELSE ${taskClaimExpiresAt ?? null}::timestamptz
      END,
      sync_retry_count = CASE
        WHEN ${syncRetryCount === undefined} THEN sync_retry_count
        WHEN ${Number.isInteger(syncRetryCount) && syncRetryCount >= 0} THEN ${syncRetryCount ?? null}::int
        ELSE sync_retry_count
      END,
      sync_max_retry_count = CASE
        WHEN ${syncMaxRetryCount === undefined} THEN sync_max_retry_count
        WHEN ${Number.isInteger(syncMaxRetryCount) && syncMaxRetryCount > 0} THEN ${syncMaxRetryCount ?? null}::int
      ...
      sla_deadline = CASE
        ...
        ELSE ${slaDeadline ?? null}::timestamptz
      END,
      is_escalated = CASE
      ...
      escalated_at = CASE
        ...
        ELSE ${escalatedAt ?? null}::timestamptz
      END,
      waiting_citizen_info_started_at = CASE
        ...
        ELSE ${waitingCitizenInfoStartedAt ?? null}::timestamptz
      END,
      waiting_citizen_info_deadline = CASE
        ...
        ELSE ${waitingCitizenInfoDeadline ?? null}::timestamptz
      END,
      camunda_metadata_json = CASE
        WHEN ${normalizedCamundaMetadata}::jsonb IS NULL
          THEN camunda_metadata_json
        ELSE ${normalizedCamundaMetadata}::jsonb
      END,
      camunda_error_summary = CASE
        WHEN ${clearCamundaError} = TRUE THEN NULL
        WHEN ${normalizedCamundaError}::text IS NOT NULL THEN ${normalizedCamundaError}::text
        ELSE camunda_error_summary
      END,
      status = COALESCE(${normalizedStatus}::text, status),
      closed_at = CASE
        WHEN COALESCE(${normalizedStatus}::text, status) IN ('CLOSED', 'RESOLVED')
...
export async function listProcedureRequestsForAdmin({
  limit = 100,
  assignmentScope = "all",
  ...
  const sql = ensureDatabase();
  await ensureProcedureRequestSchema();
  const normalizedScope = normalizeText(assignmentScope || "all", 24).toLowerCase();
  const normalizedRequesterUserId = normalizeText(requesterUserId, 80) || null;
  const assignmentWhereClause =
    normalizedScope === "mine" && normalizedRequesterUserId
      ? sql`inbox_owner_user_id = ${normalizedRequesterUserId}`
      : normalizedScope === "unassigned"
        ? sql`inbox_owner_user_id IS NULL`
        : normalizedRequesterUserId
          ? sql`(inbox_owner_user_id IS NULL OR inbox_owner_user_id = ${normalizedRequesterUserId})`
          : sql`inbox_owner_user_id IS NULL`;
  const rows = await sql`
    SELECT
      id,
...
export async function listProcedureRequestsForAssignee({
  assigneeUserId,
  limit = 100,
} = {}) {
  const sql = ensureDatabase();
  await ensureProcedureRequestSchema();
  const normalizedAssignee = normalizeText(assigneeUserId, 80);
  ...
  const rows = await sql`
    SELECT
      ...
      updated_at
    FROM chatbot_procedure_requests
    WHERE inbox_owner_user_id = ${normalizedAssignee}
    ORDER BY created_at DESC
    LIMIT ${Number.isInteger(limit) && limit > 0 ? Math.min(limit, 500) : 100};
  `;
  return rows.map((row) => mapProcedureRequestRow(row));
...
export async function listProcedureRequestsForCitizen({
  userId,
  limit = 100,
} = {}) {
  const sql = ensureDatabase();
...
export async function resolveFuncionarioAssignmentScopeForProcedureRequest({
  funcionarioUserId,
  procedureRequestId,
} = {}) {
  const sql = ensureDatabase();
  ...
  const normalizedFuncionario = normalizeText(funcionarioUserId, 80);
  const normalizedRequestId = normalizeText(procedureRequestId, 80);
  if (!normalizedFuncionario || !normalizedRequestId) {
    return null;
  }
  const [row] = await sql`
    SELECT
      CASE
        WHEN pr.inbox_owner_user_id = ${normalizedFuncionario} THEN ${FUNCIONARIO_ASSIGNMENT_SCOPE.ASSIGNED_TO_ME}
        WHEN pr.inbox_owner_user_id IS NULL
          AND pr.procedure_type_id IS NOT NULL
          AND EXISTS (
            SELECT 1
            FROM procedure_type_assignees pta
            WHERE pta.user_id = ${normalizedFuncionario}
              AND pta.procedure_type_id = pr.procedure_type_id
          )
          THEN ${FUNCIONARIO_ASSIGNMENT_SCOPE.AVAILABLE}
        ELSE NULL
      END AS assignment_scope
    FROM chatbot_procedure_requests pr
    WHERE pr.id = ${normalizedRequestId}
    LIMIT 1;
  `;
  const assignmentScope = normalizeText(row?.assignment_scope, 40).toLowerCase();
  if (
    assignmentScope === FUNCIONARIO_ASSIGNMENT_SCOPE.ASSIGNED_TO_ME ||
    assignmentScope === FUNCIONARIO_ASSIGNMENT_SCOPE.AVAILABLE
  ) {
...
export async function listProcedureRequestsForFuncionarioInbox({
  funcionarioUserId,
  limit = 100,
} = {}) {
  const sql = ensureDatabase();
  ...
  const rows = await sql`
    SELECT
      ...
      pr.whatsapp_wa_id,
      pr.request_code,
      pr.procedure_type_id,
      pr.procedure_code,
      pr.procedure_name,
      pr.procedure_category,
      pr.status,
      pr.summary,
      pr.collected_data_json,
      pr.camunda_process_instance_key,
      pr.camunda_process_definition_id,
      pr.camunda_process_version,
      pr.camunda_task_definition_key,
      pr.current_task_definition_key,
      pr.camunda_metadata_json,
      pr.camunda_error_summary,
      pr.task_assignee_id,
      pr.inbox_owner_user_id,
      pr.inbox_owner_assigned_at,
      pr.task_claimed_at,
      pr.task_claim_expires_at,
      pr.sync_retry_count,
      pr.sync_max_retry_count,
      pr.sync_last_retry_at,
      pr.sync_next_retry_at,
      pr.auto_sync_retry_enabled,
      pr.sla_deadline,
      pr.is_escalated,
      pr.escalated_at,
      pr.waiting_citizen_info_started_at,
      pr.waiting_citizen_info_deadline,
      pr.closed_at,
      pr.created_at,
      pr.updated_at,
      CASE
        WHEN pr.inbox_owner_user_id = ${normalizedFuncionario} THEN ${FUNCIONARIO_ASSIGNMENT_SCOPE.ASSIGNED_TO_ME}
        ELSE ${FUNCIONARIO_ASSIGNMENT_SCOPE.AVAILABLE}
      END AS assignment_scope,
      (pr.inbox_owner_user_id = ${normalizedFuncionario}) AS is_assigned_to_me,
      (pr.inbox_owner_user_id IS NULL) AS is_available_to_claim
    FROM chatbot_procedure_requests pr
    WHERE
      pr.inbox_owner_user_id = ${normalizedFuncionario}
      OR (
        pr.inbox_owner_user_id IS NULL
        AND pr.procedure_type_id IS NOT NULL
        AND EXISTS (
          SELECT 1
          FROM procedure_type_assignees pta
...
export async function claimProcedureRequestForFuncionarioInbox({
  procedureRequestId,
  funcionarioUserId,
} = {}) {
  const sql = ensureDatabase();
  ...
  const normalizedRequestId = normalizeText(procedureRequestId, 80);
  const normalizedFuncionario = normalizeText(funcionarioUserId, 80);
  const endpointAction = "FUNCIONARIO_CLAIM_EXPEDIENTE";
  if (!normalizedRequestId || !normalizedFuncionario) {
    console.info("[procedure-claim]", {
      endpointAction,
      procedureRequestId: normalizedRequestId || null,
      funcionarioId: normalizedFuncionario || null,
  ...
  const current = await getProcedureRequestById(normalizedRequestId);
  if (!current) {
    console.info("[procedure-claim]", {
      endpointAction,
      procedureRequestId: normalizedRequestId,
      funcionarioId: normalizedFuncionario,
      previousOwner: null,
      newOwner: null,
      rowsUpdated: 0,
      result: "procedure_not_found",
    });
    return { ok: false, status: 404, reason: "procedure_not_found" };
  }
  if (current.assignedToUserId && String(current.assignedToUserId) === normalizedFuncionario) {
    console.info("[procedure-claim]", {
      endpointAction,
      procedureRequestId: normalizedRequestId,
      funcionarioId: normalizedFuncionario,
      previousOwner: current.assignedToUserId,
      newOwner: current.assignedToUserId,
      rowsUpdated: 0,
      result: "idempotent",
    });
    return {
  ...
  if (current.assignedToUserId && String(current.assignedToUserId) !== normalizedFuncionario) {
    console.info("[procedure-claim]", {
      ...
      newOwner: current.assignedToUserId,
      rowsUpdated: 0,
      result: "assigned_to_other",
    });
    return { ok: false, status: 409, reason: "assigned_to_other" };
  }

  const assignmentScope = await resolveFuncionarioAssignmentScopeForProcedureRequest({
    funcionarioUserId: normalizedFuncionario,
    procedureRequestId: normalizedRequestId,
  });
  if (assignmentScope !== FUNCIONARIO_ASSIGNMENT_SCOPE.AVAILABLE) {
    console.info("[procedure-claim]", {
      endpointAction,
      procedureRequestId: normalizedRequestId,
      funcionarioId: normalizedFuncionario,
      previousOwner: current.assignedToUserId || null,
      newOwner: current.assignedToUserId || null,
      rowsUpdated: 0,
      result: "procedure_type_not_enabled",
    });
    return { ok: false, status: 403, reason: "procedure_type_not_enabled" };
  }

  const rows = await sql`
    UPDATE chatbot_procedure_requests pr
    SET
      inbox_owner_user_id = ${normalizedFuncionario},
      inbox_owner_assigned_at = NOW(),
      updated_at = NOW()
    WHERE pr.id = ${normalizedRequestId}
      AND pr.inbox_owner_user_id IS NULL
      AND pr.procedure_type_id IS NOT NULL
      AND EXISTS (
        SELECT 1
        FROM procedure_type_assignees pta
    ...
    RETURNING
      ...
      pr.created_at,
      pr.updated_at,
      ${FUNCIONARIO_ASSIGNMENT_SCOPE.ASSIGNED_TO_ME}::text AS assignment_scope,
      TRUE AS is_assigned_to_me,
      FALSE AS is_available_to_claim;
  `;
  const [row] = rows;
  const rowsUpdated = rows.length;
  if (row) {
    const procedureRequest = mapProcedureRequestRow(row);
    console.info("[procedure-claim]", {
      endpointAction,
      procedureRequestId: normalizedRequestId,
      funcionarioId: normalizedFuncionario,
      previousOwner: current.assignedToUserId || null,
      newOwner: procedureRequest.assignedToUserId || null,
      rowsUpdated,
      result: "claimed",
    });
    return {
  ...
  const latest = await getProcedureRequestById(normalizedRequestId);
  if (latest?.assignedToUserId && String(latest.assignedToUserId) === normalizedFuncionario) {
    console.info("[procedure-claim]", {
      endpointAction,
      procedureRequestId: normalizedRequestId,
      funcionarioId: normalizedFuncionario,
      previousOwner: current.assignedToUserId || null,
      newOwner: latest.assignedToUserId,
      rowsUpdated,
      result: "idempotent_after_race",
    });
    return {
      ok: true,
      idempotent: true,
  ...
  if (latest?.assignedToUserId && String(latest.assignedToUserId) !== normalizedFuncionario) {
    console.info("[procedure-claim]", {
      ...
      newOwner: latest.assignedToUserId,
      rowsUpdated,
      result: "assigned_to_other_after_race",
    });
    return { ok: false, status: 409, reason: "assigned_to_other" };
  }
  console.info("[procedure-claim]", {
    ...
    funcionarioId: normalizedFuncionario,
    previousOwner: current.assignedToUserId || null,
    newOwner: latest?.assignedToUserId || null,
    rowsUpdated,
    result: "claim_conflict",
  });
  return { ok: false, status: 409, reason: "claim_conflict" };
}
...
export async function deleteProcedureRequestById(procedureRequestId) {
  const sql = ensureDatabase();
  await ensureProcedureRequestSchema();
  const normalizedId = normalizeText(procedureRequestId, 80);
  if (!normalizedId) {
  ...
  const rows = await sql`
    DELETE FROM chatbot_procedure_requests
    WHERE id = ${normalizedId}
    RETURNING id;
  `;
  if (!rows.length) {
  ...
  return { ok: true, deleted: true, reason: "deleted", id: rows[0].id };
}
