...
async function ensureChatSessionSchema() {
  ...
  if (!schemaReadyPromise) {
    schemaReadyPromise = (async () => {
      ...
      await sql`
        ALTER TABLE chatbot_sessions
        ADD COLUMN IF NOT EXISTS collected_data_json JSONB NOT NULL DEFAULT '{}'::jsonb;
      `;
      await sql`
        ALTER TABLE chatbot_sessions
        ADD COLUMN IF NOT EXISTS last_interpretation_json JSONB NOT NULL DEFAULT '{}'::jsonb;
      `;
      await sql`
        ALTER TABLE chatbot_sessions
