...
export const FLOW_KEY_STATUS = "status.check";
...
/** Meta `wa_id` / número de contacto WhatsApp (solo dígitos, estable por sesión). */
...
function normalizeCurrentStep(step) {
  ...
  if (/^[a-z0-9_]{1,60}$/u.test(normalized)) {
    return normalized;
  }
...
function normalizeFlowKey(value) {
  ...
  if (normalized === FLOW_KEY_STATUS || normalized === "check_status" || normalized === "status") {
    return FLOW_KEY_STATUS;
  }
...
function normalizeIncidentRequiredFields(value) {
  ...
  const hasDefinitionObjects = value.some((item) => item && typeof item === "object");
  if (hasDefinitionObjects) {
    return normalizeProcedureFieldDefinitions(value);
  }

  const out = [];
  const seen = new Set();
  value.forEach((rawField, index) => {
    const key = normalizeStringField(rawField, 60)
      .toLowerCase()
      .replace(/[^a-z0-9_]/g, "");
    if (!key || seen.has(key)) {
      return;
    ...
    seen.add(key);
    const type = key === "photo" ? "image" : key === "location" ? "location" : "text";
    out.push({
      key,
      label: key,
      prompt: "",
...
function normalizeCollectedData(collectedData) {
  if (!collectedData || typeof collectedData !== "object") {
    return {
      ...
      // Deprecated alias: use procedureFieldDefinitions.
  ...
  const procedureFieldDefinitions = normalizeProcedureFieldDefinitions(
    collectedData.procedureFieldDefinitions || collectedData.procedureRequiredFields || []
  );
  const incidentRequiredFields = normalizeIncidentRequiredFields(collectedData.incidentRequiredFields || []);
  const lat = parseNullableFiniteNumber(collectedData.locationLatitude);
  const lng = parseNullableFiniteNumber(collectedData.locationLongitude);
  const normalized = {
    category: normalizeStringField(collectedData.category, 80),
    catalogItemId: normalizeStringField(collectedData.catalogItemId, 80),
    catalogItemCode: normalizeStringField(collectedData.catalogItemCode, 120).toLowerCase(),
    incidentRequiredFields,
    ...
    photoWhatsappMediaId: normalizeStringField(collectedData.photoWhatsappMediaId, 120),
    photoAttachmentChannel: normalizeStringField(collectedData.photoAttachmentChannel, 40),
    photoDownloadStatus: normalizeStringField(collectedData.photoDownloadStatus, 40),
    photoCaption: normalizeStringField(collectedData.photoCaption, 500),
    photoDownloadError: normalizeStringField(collectedData.photoDownloadError, 200),
    locationLatitude: lat,
    locationLongitude: lng,
    locationAddressText: normalizeStringField(collectedData.locationAddressText, 400),
    locationSource: normalizeStringField(collectedData.locationSource, 80),
    ...
    selectedProcedureCode: normalizeStringField(collectedData.selectedProcedureCode, 120).toLowerCase(),
    pendingProcedureCode: normalizeStringField(collectedData.pendingProcedureCode, 120).toLowerCase(),
    procedureDraft: normalizeStringField(collectedData.procedureDraft, 400),
    draft: normalizeStringField(collectedData.draft, 400),
    requiredFields: normalizeProcedureFieldDefinitions(
      collectedData.requiredFields || collectedData.procedureFieldDefinitions || collectedData.procedureRequiredFields || []
    ),
    missingFields: Array.isArray(collectedData.missingFields)
      ? collectedData.missingFields
          .map((item) => normalizeStringField(item, 60).toLowerCase())
          .filter(Boolean)
      : [],
    procedureFieldDefinitions,
    // Deprecated alias for compatibility with previous snapshots.
    procedureRequiredFields: procedureFieldDefinitions,
  };

  if (procedureFieldDefinitions.length > 0) {
    const catalogNormalized = normalizeProcedureCollectedData({
      ...collectedData,
      procedureFieldDefinitions,
      procedureRequiredFields: procedureFieldDefinitions,
    });
    procedureFieldDefinitions.forEach((field) => {
      ...
      normalized[fieldKey] = catalogNormalized[fieldKey];
    });
  }

  normalized.sttCriticalEchoPending = collectedData.sttCriticalEchoPending === true;
...
function computeMissingFields(flowKey, collectedData) {
  if (flowKey !== FLOW_KEY_INCIDENT) {
    if (flowKey === FLOW_KEY_PROCEDURE) {
      const fieldDefinitions = normalizeProcedureFieldDefinitions(
        collectedData?.procedureFieldDefinitions || collectedData?.procedureRequiredFields || []
      );
      if (fieldDefinitions.length > 0) {
        return getProcedureMissingFieldsFromDefinition(fieldDefinitions, collectedData || {});
      }
  ...
  const requiredIncidentFields = normalizeIncidentRequiredFields(collectedData?.incidentRequiredFields || []);
  const requiredDefinitions =
    requiredIncidentFields.length > 0 ? requiredIncidentFields : DEFAULT_INCIDENT_REQUIRED_FIELDS;
  return requiredDefinitions
    .filter((field) => field.required !== false)
    .map((field) => field.key)
    .filter((fieldKey) => {
      const fieldDefinition =
        requiredDefinitions.find((field) => field.key === fieldKey) || null;
      if (fieldDefinition?.type === "image" && fieldKey === "photo") {
        const photoStatus = normalizePhotoStatus(collectedData?.photoStatus);
...
function normalizeSessionSnapshot(snapshot) {
  ...
  const confirmationContextRaw =
    snapshot?.confirmationContext && typeof snapshot.confirmationContext === "object"
      ? snapshot.confirmationContext
      : null;
  const confirmationContext = confirmationContextRaw
    ? {
        procedureCode: normalizeStringField(confirmationContextRaw.procedureCode, 120).toLowerCase(),
        procedureName: normalizeStringField(confirmationContextRaw.procedureName, 160),
        collectedDataSnapshot: normalizeCollectedData(confirmationContextRaw.collectedDataSnapshot || {}),
        shownAt: normalizeIsoDate(confirmationContextRaw.shownAt),
      }
    : null;
  ...
  return {
    ...
    whatsappWaId: normalizeWhatsAppWaId(snapshot?.whatsappWaId),
    ...
    awaitingFinalConfirmation: flowKey ? snapshot?.awaitingFinalConfirmation === true : false,
    confirmationContext:
      flowKey &&
      confirmationContext &&
      (confirmationContext.procedureCode ||
        confirmationContext.procedureName ||
        Object.keys(confirmationContext.collectedDataSnapshot || {}).length > 0 ||
        confirmationContext.shownAt)
        ? confirmationContext
        : null,
    collectedData,
  ...
  if (!schemaReadyPromise) {
    schemaReadyPromise = (async () => {
      ...
      const schemaStatements = [
        ...
        `ALTER TABLE chatbot_sessions ADD COLUMN IF NOT EXISTS whatsapp_wa_id TEXT;`,
        ...
        `ALTER TABLE chatbot_sessions ADD COLUMN IF NOT EXISTS awaiting_final_confirmation BOOLEAN;`,
        `UPDATE chatbot_sessions SET awaiting_final_confirmation = FALSE WHERE awaiting_final_confirmation IS NULL;`,
        `ALTER TABLE chatbot_sessions ALTER COLUMN awaiting_final_confirmation SET DEFAULT FALSE;`,
        `ALTER TABLE chatbot_sessions ALTER COLUMN awaiting_final_confirmation SET NOT NULL;`,
        `ALTER TABLE chatbot_sessions ADD COLUMN IF NOT EXISTS confirmation_context_json JSONB;`,
  ...
  try {
    ...
    if (row) {
      ...
      const normalizedSnapshot = normalizeSessionSnapshot({
        ...
        awaitingFinalConfirmation: row.awaiting_final_confirmation === true,
        confirmationContext: parseJsonFromDatabase(row.confirmation_context_json),
        collectedData: {
  ...
  const persistWithUserId = async (userIdValue) => {
    ...
    await sql`
      ...
      VALUES (
        ${sessionId},
        ${userIdValue},
        ${snapshot.whatsappWaId ?? null},
        ${snapshot.locale},
        ${snapshot.state},
        ${snapshot.flowKey},
        ${snapshot.currentStep},
        ${snapshot.confirmationState},
        ${snapshot.awaitingFinalConfirmation === true},
        ${snapshot.confirmationContext ? JSON.stringify(snapshot.confirmationContext) : null}::jsonb,
      ...
      DO UPDATE SET
        ...
        awaiting_final_confirmation = EXCLUDED.awaiting_final_confirmation,
        confirmation_context_json = EXCLUDED.confirmation_context_json,
