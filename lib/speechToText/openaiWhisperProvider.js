...
import { getSttOpenAIClient } from "../openai/openaiClientRegistry";
import { createTranscriptionWithLogs } from "../openai/openaiLoggedClient";
 ...
 * @param {string} [input.openAiLogContext] — etiqueta para logs JSON (p. ej. whatsapp.audio.transcription)
...
export async function transcribeWithOpenAIWhisper(input) {
  const client = getSttOpenAIClient();
  if (!client) {
  ...
  const logContext =
    typeof input.openAiLogContext === "string" && input.openAiLogContext.trim()
      ? input.openAiLogContext.trim()
  ...
  try {
    ...
    const payloadForLog = {
      ...
      file: "[omitted_large_content]",
    };

    let result;
    try {
      result = await createTranscriptionWithLogs(client, {
        context: logContext,
        params: {
          file,
          model,
      ...
      result = await createTranscriptionWithLogs(client, {
        context: `${logContext}.retry_plain`,
        params: { file, model },
        payloadForLog: { ...payloadForLog, response_format: null, note: "retry_without_json_format" },
        requestOptions: createOpts,
      });
    }
    ...
    console.info("[whatsapp] audio STT: transcript summary", {
      model,
