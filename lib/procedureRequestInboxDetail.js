...
export function buildAvailableActions({
  ...
  const resolvedAssignmentScope =
    assignmentScope ||
    (procedureRequest?.isAssignedToMe
      ? "assigned_to_me"
      : procedureRequest?.isAvailableToClaim
        ? "available"
        : procedureRequest?.assignmentScope || null);
  if (requestsApiSegment === "funcionario" && resolvedAssignmentScope === "available") {
    actions.push({
      actionKey: "claim_task",
      label: "Tomar expediente",
      displayLabel: "Tomar expediente",
      endpoint: `${basePath}/claim-expediente`,
      method: "POST",
  ...
  const fieldDefinitions = Array.isArray(procedureType?.fieldDefinitions)
    ? procedureType.fieldDefinitions
    : Array.isArray(procedureType?.requiredFields)
      ? procedureType.requiredFields
      : [];
  ...
  if (includeClaimTask && activeTask?.taskDefinitionKey && !procedureRequest?.taskAssigneeId) {
    const claimTaskEndpoint =
      requestsApiSegment === "funcionario" ? `${basePath}/claim-camunda-task` : `${basePath}/claim-task`;
    actions.push({
      actionKey: "claim_task",
      label: `Reclamar tarea: ${taskDisplayName}`,
      displayLabel: "Reclamar tarea",
      endpoint: claimTaskEndpoint,
      method: "POST",
      requiresTaskClaim: false,
      requiredVariables: [],
    });
      ...
      configuredRequiredVariables.length > 0
        ? configuredRequiredVariables.map((configuredVariable) => {
            ...
            const fieldLabel =
              ...
              fieldDefinitions.find((field) => field?.key === procedureFieldKey)?.label ||
              procedureFieldKey ||
              configuredCamundaVariableName ||
              "Variable";
            return {
        ...
        : completeTaskMappings.map((mapping) => ({
            ...
            fieldLabel:
              fieldDefinitions.find((field) => field?.key === mapping.procedureFieldKey)?.label ||
