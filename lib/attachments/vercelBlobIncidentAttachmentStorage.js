...
export function createVercelBlobIncidentAttachmentStorage() {
  const blobAccess = getVercelBlobAccessMode();
  ...
  return {
    ...
    async uploadDraftAttachment({ sessionId, bytes, mimeType }) {
      ...
      if (!ext) {
        return {
          ok: false,
          error: "Tipo de archivo no permitido. Usá JPG, PNG, WebP, GIF, BMP, SVG, AVIF, HEIC o HEIF.",
        };
      }
      ...
      try {
        ...
        return {
          ...
          publicUrl: blobAccess === "public" ? out.url : null,
          sizeBytes: typeof out.size === "number" ? out.size : bytes.length,
        };
      } catch (error) {
        const msg = typeof error?.message === "string" ? error.message : "";
        console.error("[vercelBlobAttachment] put falló", { pathname, message: msg });
        if (msg.includes("private store") && blobAccess === "public") {
          console.error(
            "[vercelBlobAttachment] El Blob Store es privado: en Vercel definí ATTACHMENT_VERCEL_BLOB_ACCESS=private y redeploy."
          );
        }
        return {
          ok: false,
    ...
    async readDraftAttachmentBytes(draft) {
      try {
        const res = await get(draft.publicUrl || draft.storageKey, { access: blobAccess });
    ...
    async readFinalIncidentBytes({ storageKey, publicUrl, mimeType }) {
      try {
        const res = await get(publicUrl || storageKey, { access: blobAccess });
        if (!res || res.statusCode !== 200 || !res.stream) {
          return null;
        }
        const buffer = await webReadableStreamToBuffer(res.stream);
