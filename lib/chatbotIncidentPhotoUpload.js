...
async function persistProcedureFlowPhotoForChatSession({
  ...
  const registeredAttachment = await registerProcedureImageAttachment({
    sourceChannel: snapshot?.whatsappWaId ? "whatsapp" : "web",
    sessionId,
    bytes,
    mimeType: String(mimeType || "").toLowerCase().trim(),
    originalName,
  });
  if (!registeredAttachment.ok) {
    return { status: 400, body: { error: registeredAttachment.error } };
  }

  const { attachmentData, canonicalImage, storage, uploadResult } = registeredAttachment;
  const nextCollected = {
    ...procedureData,
  ...
  try {
    updatedSnapshot = await setConversationState(
      ...
      createProcedureFlowSnapshotPatch({
        ...
        confirmationContext:
          nextStep === CHATBOT_CURRENT_STEPS.CONFIRMATION
            ? {
                procedureCode: normalizeNameField(nextCollected.procedureCode, 120).toLowerCase(),
                procedureName: normalizeNameField(nextCollected.procedureName, 160),
                collectedDataSnapshot: nextCollected,
    ...
    try {
      await storage.deleteDraftAttachment({
        ...
        mimeType: attachmentData.photoAttachmentContentType || "application/octet-stream",
...
export async function persistProcedurePhotoForChatSession({
  ...
  const { attachmentData, storage, uploadResult } = registeredAttachment;
  const nextCollected = {
