...
async function persistProcedurePhotoForChatSession({
  sessionId,
  userId,
  bytes,
  mimeType,
  ...
  const requiredFields = Array.isArray(procedureData.procedureRequiredFields)
    ? procedureData.procedureRequiredFields
    : [];

  const normalizedCurrentStep = normalizeNameField(snapshot?.currentStep, 80).toLowerCase();
  const fieldFromStep = getProcedureFieldDefinition(requiredFields, normalizedCurrentStep);
  const missingFieldsBefore = getProcedureMissingFieldsFromDefinition(requiredFields, procedureData);
  const firstMissingField = missingFieldsBefore[0] || null;
  const fieldFromMissing = getProcedureFieldDefinition(requiredFields, firstMissingField);
  const targetField =
  ...
  const missingFieldsAfter = getProcedureMissingFieldsFromDefinition(requiredFields, nextCollected);
  ...
  try {
    ...
    console.error("[chatbotIncidentPhotoUpload] Falló la actualización de sesión (procedure); se revierte el adjunto.", {
      sessionId,
  ...
  const replyText =
    nextStep === CHATBOT_CURRENT_STEPS.CONFIRMATION
      ...
      : buildProcedureFieldPrompt(getProcedureFieldDefinition(requiredFields, nextStep));
...
export async function persistIncidentPhotoForChatSession({
  ...
  if (snapshot.flowKey === FLOW_KEY_PROCEDURE) {
    return persistProcedurePhotoForChatSession({
      sessionId,
      userId,
      bytes,
      mimeType,
