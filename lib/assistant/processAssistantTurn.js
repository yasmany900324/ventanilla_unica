...
function buildIntentClarificationReply() {
  const lines = [
    "Puedo ayudarte con:",
    "",
    "1. Iniciar un trámite o reporte",
...
function buildProcedureCatalogIntroReply(procedures) {
  ...
  const names = supported
    .slice(0, 6)
    .map((procedure) => normalizeStringField(procedure?.name, 160))
    .filter(Boolean);
  if (names.length === 0) {
    return buildNoActiveProceduresReply();
  }
...
function buildUnsupportedProcedureReply() {
  return "No encontré una coincidencia clara con los procedimientos disponibles.";
}
...
export async function processAssistantTurn({
  ...
  if (
    ...
    const procedureSelection = await resolveProcedureCatalogSelection({
      text,
      activeProcedures,
      interpretation,
    });
    const procedureMatch = procedureSelection.procedure;
    if (chatDebugEnabled) {
      logChatDebug(
