...
export async function processAssistantTurn({
  ...
  if (isIncidentContextStart) {
    ...
    if (nextStep === CHATBOT_CURRENT_STEPS.CONFIRMATION) {
      ...
      return buildAssistantTurnResult({
        sessionId,
        locale: effectiveLocale,
        replyText: buildIncidentConfirmationIntroReply(effectiveLocale),
    ...
    return buildAssistantTurnResult({
      ...
      replyText:
        nextStep === CHATBOT_CURRENT_STEPS.DESCRIPTION
          ...
          : buildQuestionForStep({ step: nextStep }),
      snapshot: savedSnapshot,
  ...
  if (isProcedureFlowActive(snapshot)) {
    ...
    if (procedureSwitchToIncident) {
      ...
      return buildAssistantTurnResult({
        sessionId,
        locale: effectiveLocale,
        replyText: buildQuestionForStep({ step: nextIncidentStep }),
        snapshot: switchedSnapshot,
        actionOptions:
  ...
  if (
    ...
    return buildAssistantTurnResult({
      ...
      locale: effectiveLocale,
      replyText:
        "Para crear la incidencia, confirmá con «Confirmar y crear incidencia» o escribí sí, dale o de acuerdo. Si necesitás cambiar algo, tocá «Corregir datos» o decime qué dato querés corregir.",
      snapshot,
      actionOptions: buildConfirmationActionOptions(),
      nextStepType: "confirm_incident",
      nextStepField: null,
