...
export async function processAssistantTurn({
  ...
  if (procedureOnlyCatalogMode && snapshot?.flowKey === FLOW_KEY_INCIDENT) {
    snapshot = await setConversationState(sessionId, {
      locale: effectiveLocale,
      ...
      return buildAssistantTurnResult({
        ...
        replyText:
          procedureSelection.status === "invalid_number"
            ...
            : procedureOnlyCatalogMode
              ? buildUnsupportedProcedureReply()
    ...
    if (procedureMissing.length === 0) {
      const replyText = needsSttProcedureEcho
        ...
        : hasProcedureUpdate
          ...
          : "Ya tengo los datos de este trámite. Respondé sí para confirmar, no para cancelar, o escribí qué dato querés corregir.";
      return buildAssistantTurnResult({
        sessionId,
        locale: effectiveLocale,
        replyText,
