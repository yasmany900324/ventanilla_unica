...
import {
  ...
  buildIncidentCorrectionMenuActionOptions,
  buildIncidentCorrectionMenuReply,
...
function getRequiredMissingFields(collectedData) {
  const configuredFields = Array.isArray(collectedData?.incidentRequiredFields)
    ? collectedData.incidentRequiredFields
        .map((field) => normalizeStringField(field, 60).toLowerCase())
        .filter((field) => field === "location" || field === "description" || field === "risk")
    : [];
  const requiredFields = configuredFields.length > 0 ? configuredFields : ["location", "description", "risk"];
  const missing = [];
  if (requiredFields.includes("location") && !collectedData?.location) {
    missing.push("location");
  }
  if (requiredFields.includes("description") && !collectedData?.description) {
    missing.push("description");
  }
  if (requiredFields.includes("risk") && !collectedData?.risk) {
    missing.push("risk");
  }
  return missing;
}
...
function mergeIncidentCatalogSelection(collectedData, catalogItem) {
  ...
  const requiredFields = Array.isArray(catalogItem.requiredFields)
    ? catalogItem.requiredFields
        ...
        .map((field) => normalizeStringField(field?.key, 60).toLowerCase())
        .filter((field) => field === "location" || field === "description" || field === "risk")
    : [];
  const next = {
    ...base,
...
function buildProcedureCatalogIntroReply(procedures) {
  ...
  return `Puedo guiarte con ${totalSupported} trámites: ${listedNames}. Elegí uno en los botones o decime cuál necesitás.`;
}
...
export async function processAssistantTurn({
  ...
  if (isIncidentContextStart) {
    ...
    return buildAssistantTurnResult({
      sessionId,
      locale: effectiveLocale,
      replyText: buildQuestionForStep({ step: nextStep, channel: copyChannel }),
      snapshot: savedSnapshot,
      actionOptions,
  ...
  if (effectiveCommand === "open_incident_correction_menu") {
    ...
    if (
      ...
      return buildAssistantTurnResult({
        ...
        locale: effectiveLocale,
        replyText:
          "Cuando lleguemos al resumen final del reporte vas a poder corregir ubicación, descripción, riesgo o foto desde «Corregir datos».",
        snapshot,
        actionOptions: [],
    ...
    return buildAssistantTurnResult({
      sessionId,
      locale: effectiveLocale,
      replyText: buildIncidentCorrectionMenuReply(),
      snapshot,
      actionOptions: buildIncidentCorrectionMenuActionOptions(),
      nextStepType: "incident_correction_menu",
      nextStepField: null,
    });
  }
  ...
  if (
    ...
    return buildAssistantTurnResult({
      sessionId,
      locale: effectiveLocale,
      replyText: buildQuestionForStep({ step: targetStep, channel: copyChannel }),
  ...
  if (effectiveCommand === "confirm" && isIncidentFlowActive(snapshot)) {
    ...
    try {
      ...
      await syncIncidentToCamundaAfterCreate(incident, {
        ...
        risk: typeof snapshot.collectedData?.risk === "string" ? snapshot.collectedData.risk : null,
  ...
  if (switchToIncident && !isIncidentFlowActive(snapshot) && !isProcedureFlowActive(snapshot)) {
    ...
    const hasDescription = Boolean(mergedWithCatalog?.description);
    const nextStep = hasDescription
      ? getNextIncidentFlowStep(mergedWithCatalog)
    ...
    return buildAssistantTurnResult({
      ...
      replyText:
        nextStep === CHATBOT_CURRENT_STEPS.DESCRIPTION
          ...
          : buildQuestionForStep({ step: nextStep, channel: copyChannel }),
      snapshot: savedSnapshot,
  ...
  if (switchToProcedure && !isProcedureFlowActive(snapshot) && !isIncidentFlowActive(snapshot)) {
    ...
    return buildAssistantTurnResult({
      sessionId,
      locale: effectiveLocale,
      replyText: `${buildProcedureStartReply()} ${buildProcedureFieldPrompt(fieldDefinition)}`.replace(/\s+/g, " ").trim(),
      snapshot: procedureSnapshot,
      actionOptions: buildProcedureActionOptions({
  ...
  if (isProcedureFlowActive(snapshot)) {
    ...
    if (procedureSwitchToIncident) {
      ...
      return buildAssistantTurnResult({
        sessionId,
        locale: effectiveLocale,
        replyText: buildQuestionForStep({ step: nextIncidentStep, channel: copyChannel }),
        snapshot: switchedSnapshot,
        actionOptions:
    ...
    if (
      ...
      return buildAssistantTurnResult({
        ...
        locale: effectiveLocale,
        replyText:
          "Para registrar la solicitud, usá «Confirmar datos del trámite» o escribí «sí», «dale» o «de acuerdo».",
        snapshot,
        actionOptions: buildProcedureActionOptions({ isCompleted: true }),
        nextStepType: "procedure_confirm",
        nextStepField: null,
    ...
    if (procedureMissing.length === 0) {
      const replyText = needsSttProcedureEcho
        ...
        : hasProcedureUpdate
          ...
          : "Ya tengo los datos de este trámite. Si está correcto, confirmá para continuar.";
      return buildAssistantTurnResult({
        sessionId,
        locale: effectiveLocale,
        replyText,
  ...
  if (!isIncidentFlowActive(snapshot) && !isProcedureFlowActive(snapshot)) {
    ...
    if (!shouldActivate) {
      ...
      return buildAssistantTurnResult({
        ...
        locale: effectiveLocale,
        replyText:
          "¿Preferís reportar una incidencia o iniciar un trámite? Elegí una opción abajo o decime cuál.",
        snapshot: {
          ...snapshot,
          locale: effectiveLocale,
          lastInterpretation: interpretation,
  ...
  if (channel === "whatsapp") {
    ...
    if (mergedResult.acceptedEntities.includes("risk")) {
      ackParts.push(`Nivel de riesgo registrado: ${mergedDataWithCatalog.risk}.`);
    }
