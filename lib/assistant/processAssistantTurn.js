...
export async function processAssistantTurn({
  ...
  const shouldSkipLlmForStatusWaiting =
    ...
    typeof text === "string" &&
    text.trim().length > 0 &&
    isStatusFlowWaitingCaseCode(snapshot) &&
    !isIncidentFlowActive(snapshot) &&
    !isProcedureFlowActive(snapshot);
  ...
  const statusFlowActive = isStatusFlowWaitingCaseCode(snapshot);
  ...
  if (
    ...
    return buildAssistantTurnResult({
      sessionId,
      locale: effectiveLocale,
      replyText: buildStatusReply({ channel, identifierRequested: statusFlowActive }),
