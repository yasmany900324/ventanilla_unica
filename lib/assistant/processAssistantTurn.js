...
export async function processAssistantTurn({
  ...
  if (effectiveCommand === "confirm" && isIncidentFlowActive(snapshot)) {
    ...
    try {
      ...
      const attachmentFromChatDraft =
        draft.photoStatus === "provided" && draft.photoAttachmentStoredFilename
          ? {
              storedFilename: draft.photoAttachmentStoredFilename,
  ...
  if (
    ...
    return buildAssistantTurnResult({
      ...
      locale: effectiveLocale,
      replyText:
        "Para crear la incidencia, usá «Confirmar y crear incidencia» o escribí «sí», «dale» o «de acuerdo». Si necesitás corregir algo, tocá Editar.",
      snapshot,
      actionOptions: buildConfirmationActionOptions(),
      nextStepType: "confirm_incident",
      nextStepField: null,
