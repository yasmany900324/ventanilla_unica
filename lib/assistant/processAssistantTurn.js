...
function hasPendingFinalConfirmation(snapshot) {
  ...
  const hasPromptFallback =
    typeof context?.lastAssistantPrompt === "string" &&
    context.lastAssistantPrompt.includes("¿Confirmás que registre esta incidencia con estos datos?");
  if (hasPromptFallback) {
    return true;
  }
  if (isProcedureFlowActive(snapshot) && hasPendingProcedureConfirmation(snapshot)) {
    return true;
  }
  if (
