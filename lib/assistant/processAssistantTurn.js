 ...
 * @param {object|null} [params.authenticatedUser] — portal user when known (web cookie); null on WhatsApp until linked
...
export async function processAssistantTurn({
  ...
  if (channel === "whatsapp") {
    ...
    if (mergedResult.acceptedEntities.includes("description")) {
      ackParts.push(`Descripción registrada: ${mergedData.description}.`);
    }
    if (mergedResult.acceptedEntities.includes("risk")) {
      ackParts.push(`Nivel de riesgo registrado: ${mergedData.risk}.`);
    }
    if (mergedResult.acceptedEntities.includes("location")) {
      ackParts.push(`Ubicación registrada: ${mergedData.location}.`);
    }
    if (mergedResult.acceptedEntities.includes("photo")) {
      const ps = mergedData.photoStatus;
      if (ps === "skipped") {
