...
describe("buildIncidentConfirmationGateReply", () => {
  it("whatsapp alinea el tono sin frases rígidas ni listas de comandos", () => {
    const msg = buildIncidentConfirmationGateReply("whatsapp");
    expect(msg.toLowerCase()).not.toContain("corregir datos");
    expect(msg.toLowerCase()).not.toContain("escribí confirmar");
    expect(msg.toLowerCase()).not.toContain("«confirmar»");
    expect(msg).toContain("sigo con el registro");
  });

  it("web evita citar etiquetas de botón y listas mecánicas", () => {
    const msg = buildIncidentConfirmationGateReply("web");
    expect(msg.toLowerCase()).not.toContain("corregir datos");
    expect(msg.toLowerCase()).not.toContain("confirmar y crear incidencia");
    expect(msg.toLowerCase()).not.toContain("«corregir");
  });
});
...
describe("buildIncidentConfirmationIntroReply — web vs WhatsApp", () => {
  ...
  it("con channel whatsapp incluye resumen en texto plano", () => {
    ...
    expect(intro).toContain("- Evidencia: sin foto adjunta");
    expect(intro).toContain("Si está todo correcto, sigo con el registro. ¿Te parece bien?");
    expect(intro).not.toContain("confirmar la incidencia");
    expect(intro).not.toContain("corregir datos");
  });
});
