...
describe("buildIncidentConfirmationIntroReply — web vs WhatsApp", () => {
  ...
  it("en el resumen prioriza el nombre visible persistido sobre el nombre interno", () => {
    const intro = buildIncidentConfirmationIntroReply("es", {
      channel: "whatsapp",
      collectedData: {
        ...
        category: "incidencia_general",
        subcategory: "reporte_general",
        description: "Árbol caído en la vereda",
        incidentRequiredFields: [
          { key: "description", label: "Descripción", type: "text", required: true, order: 1 },
        ],
      },
    });
    expect(intro).toContain("- Procedimiento: Reportar un problema común");
    expect(intro).not.toContain("- Procedimiento: Registrar incidencia");
  });
});
