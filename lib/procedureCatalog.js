...
const DEFAULT_CASE_CATALOG = [
  ...
  {
    code: "inc_arbol_caido",
    name: "Árbol caído o ramas peligrosas",
    description:
    ...
    aliases: ["árbol caído", "arbol caido", "rama caída", "ramas peligrosas"],
    keywords: ["árbol", "rama", "caído", "peligro", "calle", "vereda"],
    requiredFields: [
      {
        key: "location",
        label: "ubicación",
        prompt: "Indicá la ubicación exacta o referencia del árbol/rama.",
        type: "text",
        required: true,
      ...
      },
      {
        key: "description",
        label: "descripción",
        prompt: "Contame qué está pasando con el árbol o las ramas.",
        type: "text",
        required: true,
      ...
      },
      {
        key: "risk",
        label: "riesgo",
        prompt: "Indicá el nivel de riesgo (alto, medio o bajo).",
        type: "select",
        required: true,
    ...
    flowDefinition: {
      completionMessage:
        "Ya registré la información base de la incidencia. Ahora vamos a confirmar los datos para enviarla.",
    },
  },
];
...
export function buildProcedureDraftConfirmationText({ procedureName, requiredFields, collectedData }) {
  ...
  if (pending.length === 0) {
    return `Tengo los datos cargados para «${normalizedProcedureName}». Si es correcto, confirmá para seguir con la solicitud.`;
  }
