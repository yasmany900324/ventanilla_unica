...
async function seedIncidentTaskUiDictionary(sql) {
  const defaultProcedureEntry = DEFAULT_CASE_CATALOG.find(
    (entry) => normalizeText(entry?.caseType, 40) === "procedure"
  );
  const defaultProcedureCode = normalizeText(defaultProcedureEntry?.code, 120);
  if (!defaultProcedureCode) {
    return false;
  }
  const [procedureRow] = await sql`
    SELECT id, flow_definition_json
    FROM chatbot_procedure_catalog
    WHERE code = ${defaultProcedureCode}
...
function isDataOrBase64LikeString(value) {
  if (typeof value !== "string") {
    return false;
  }
  const trimmed = value.trim();
  ...
  // High-entropy payloads (possible base64 blobs) must not be persisted in collectedData.
  return /^[A-Za-z0-9+/=]{160,}$/.test(trimmed);
}

function sanitizeAttachmentStorageString(value, maxLength) {
  const normalized = normalizeText(value, maxLength);
  if (!normalized || isDataOrBase64LikeString(normalized)) {
    return "";
  }
  return normalized;
}

function sanitizeAttachmentPublicUrl(value) {
  const normalized = normalizeText(value, 2048);
  if (!normalized || isDataOrBase64LikeString(normalized) || !looksLikeHttpUrl(normalized)) {
    return "";
  }
  try {
...
function sanitizeAttachmentSizeBytes(value) {
  const n =
  ...
  // Defensive cap (50 MB) for metadata sanity.
  return Math.min(int, 50 * 1024 * 1024);
}
...
function hasMeaningfulLocationLabelOrAddress(address, label) {
  return normalizeText(address, 500).length >= 3 || normalizeText(label, 300).length >= 3;
}

function hasTrustedLocationEvidence(value) {
  const source = normalizeLookup(
    normalizeText(value?.locationSource, 80) ||
      normalizeText(value?.source, 80) ||
      normalizeText(value?.origin, 80)
  );
  if (!source) {
    return false;
  }
...
export function buildCanonicalLocationFieldValue(rawData, procedureFieldKey, locationFieldDefinitions) {
  ...
  const direct = getCollectedDataValueByFieldKey(raw, key);
  const trustedSource = hasTrustedLocationEvidence(raw);
  let lat = null;
  let lng = null;
  let address = "";
  let label = "";
  let locationSource = normalizeText(raw.locationSource, 80);
  ...
  if (typeof direct === "string" && direct.trim()) {
    ...
    locationSource =
      normalizeText(direct.locationSource, 80) ||
      normalizeText(direct.source, 80) ||
      normalizeText(direct.origin, 80) ||
      locationSource;
  }
  ...
  if (lat != null && lng != null && isZeroCoordinatePair(lat, lng) && !trustedSource) {
    lat = null;
    lng = null;
  }
  ...
  if (locationSource) {
    out.locationSource = locationSource;
  }
  ...
  if (type === "location") {
    if (value && typeof value === "object" && !Array.isArray(value)) {
      ...
      if (lat != null && lng != null) {
        if (isZeroCoordinatePair(lat, lng) && !hasTrustedLocationEvidence(value)) {
          return false;
        }
        return true;
      }
      ...
      const label =
        typeof value.label === "string"
          ...
          : typeof value.description === "string"
            ...
            : "";
      if (!hasMeaningfulLocationLabelOrAddress(addr, label)) {
        return false;
      }
      return hasTrustedLocationEvidence(value) || typeof value.address === "string";
    }
    if (typeof value === "string") {
      ...
      return !/^(?:coordenadas:)?\s*0(?:[\.,]0+)?\s*,\s*0(?:[\.,]0+)?$/iu.test(compact);
    }
    return false;
  }
...
export function normalizeProcedureCollectedData(collectedData) {
  ...
  const normalizedProcedureCode = normalizeLookup(rawData.procedureCode)
    ...
    .replace(/[^a-z0-9_]/g, "")
    .slice(0, 120);
  const normalizedSelectedProcedureCode = normalizeLookup(rawData.selectedProcedureCode)
    .replace(/[\s-]+/g, "_")
    .replace(/[^a-z0-9_]/g, "")
    .slice(0, 120);
  const normalizedPendingProcedureCode = normalizeLookup(rawData.pendingProcedureCode)
    .replace(/[\s-]+/g, "_")
    .replace(/[^a-z0-9_]/g, "")
    .slice(0, 120);
  ...
  const normalized = {
    ...
    selectedProcedureCode: normalizedSelectedProcedureCode,
    pendingProcedureCode: normalizedPendingProcedureCode,
    ...
    // Attachment storage references used by funcionario preview endpoint (/photo).
    photoAttachmentStorageProvider: sanitizeAttachmentStorageString(rawData.photoAttachmentStorageProvider, 40),
    photoAttachmentStorageKey: sanitizeAttachmentStorageString(rawData.photoAttachmentStorageKey, 512),
    photoAttachmentPublicUrl: sanitizeAttachmentPublicUrl(rawData.photoAttachmentPublicUrl),
    photoAttachmentStoredFilename: sanitizeAttachmentStorageString(rawData.photoAttachmentStoredFilename, 240),
    photoAttachmentOriginalName: sanitizeAttachmentStorageString(rawData.photoAttachmentOriginalName, 240),
    photoAttachmentMimeType:
      sanitizeAttachmentStorageString(rawData.photoAttachmentMimeType, 120) ||
      sanitizeAttachmentStorageString(rawData.photoAttachmentContentType, 120),
    photoAttachmentContentType: sanitizeAttachmentStorageString(rawData.photoAttachmentContentType, 120),
    photoAttachmentSizeBytes: sanitizeAttachmentSizeBytes(rawData.photoAttachmentSizeBytes),
  };
...
export function getProcedureMissingFieldsFromDefinition(requiredFields, collectedData) {
  const normalizedFields = normalizeProcedureFieldDefinitions(requiredFields);
  const normalizedCollectedData = normalizeProcedureCollectedData({
    ...(collectedData && typeof collectedData === "object" ? collectedData : {}),
    procedureFieldDefinitions: normalizedFields,
    procedureRequiredFields: normalizedFields,
  });
  ...
  const requiredFieldKeys = normalizedFields.filter((field) => field.required !== false).map((field) => field.key);
  const collectedKeys = Object.keys(normalizedCollectedData || {});
  const evaluations = requiredFieldKeys.map((fieldKey) => {
    const field = normalizedFields.find((item) => item.key === fieldKey) || null;
    const value = field ? normalizedCollectedData[field.key] : undefined;
    const present = field ? catalogProcedureFieldHasPresentValue(field, value) : false;
    let reason = present ? "present" : "missing_or_invalid";
    if (field && normalizeLookup(field.type) === "location" && value && typeof value === "object") {
      const lat = pickFiniteNumber(value.lat ?? value.latitude);
      const lng = pickFiniteNumber(value.lng ?? value.longitude);
      if (lat != null && lng != null && isZeroCoordinatePair(lat, lng) && !hasTrustedLocationEvidence(value)) {
        reason = "invalid_default_zero_coordinates_without_user_evidence";
      }
    }
    return {
      fieldKey,
  ...
  console.info("[procedure_catalog] required fields evaluation", {
    procedureCode: normalizedCollectedData.procedureCode || "",
    requiredFields: requiredFieldKeys,
    collectedDataKeys: collectedKeys,
    evaluations,
  });

  return evaluations.filter((entry) => !entry.present).map((entry) => entry.fieldKey);
}
...
function formatProcedureFieldValueForSummary(field, rawValue) {
  ...
  if (fieldType === "image" || fieldType === "file" || fieldType === "attachment") {
    if (rawValue && typeof rawValue === "object" && !Array.isArray(rawValue)) {
      const canonical = coerceCanonicalImageOrFileObject(rawValue);
      if (canonical?.url || canonical?.filename) {
  ...
  if (fieldType === "location") {
    if (rawValue && typeof rawValue === "object" && !Array.isArray(rawValue)) {
      if (!catalogProcedureFieldHasPresentValue({ type: "location", key: field?.key || "location" }, rawValue)) {
        return "(pendiente)";
      }
...
export function buildProcedureSummaryText({ procedureName, fieldDefinitions, requiredFields, collectedData }) {
  ...
  const normalizedData = normalizeProcedureCollectedData({
    ...(collectedData && typeof collectedData === "object" ? collectedData : {}),
    procedureFieldDefinitions: normalizedFields,
    procedureRequiredFields: normalizedFields,
  });
