...
function coerceCanonicalImageOrFileObject(value) {
  ...
  const filename =
    normalizeText(value.filename, 200) || normalizeText(value.originalName, 200) || "";
...
export function buildCanonicalImageOrFileFieldValue(rawData, procedureFieldKey, _imageOrFileFieldDefinitions) {
  ...
  const mt = normalizeText(raw.photoAttachmentMimeType, 120);
  if (mt) {
    out.mimeType = mt;
  }
  const sz = Number(raw.photoAttachmentSizeBytes);
