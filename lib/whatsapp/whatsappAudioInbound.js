...
import {
  getSpeechToTextConfigDiagnostics,
  isSpeechToTextConfigured,
} from "../speechToText/resolveSpeechToTextProvider";
...
/**
 * 429 / cuota / billing de OpenAI (Whisper) — no es fallo del audio del usuario.
 * @param {string} message
 */
function isOpenAiSttApiLimitError(message) {
  ...
  if (m.includes("quota") || m.includes("billing") || m.includes("exceeded your current")) {
    return true;
  }
  return false;
}
 ...
 *   | { ok: false, reason: 'stt_disabled'|'mime_rejected'|'download'|'transcription'|'transcription_api_limit'|'empty_transcript', error?: string }
 * >}
 */
export async function processWhatsAppInboundAudioToText(params) {
  ...
  if (!isSpeechToTextConfigured()) {
    const sttDiag = getSpeechToTextConfigDiagnostics();
    console.warn("[whatsapp] audio pipeline: STT not configured", {
      ...logBase,
      sttConfigReason: sttDiag.ok ? undefined : sttDiag.reason,
      sttProviderRaw: sttDiag.sttProviderRaw,
      hasSttApiKey: sttDiag.hasSttApiKey,
      hasOpenAiApiKey: sttDiag.hasOpenAiApiKey,
    });
    await trackChatbotEvent({
      ...
      details: sttDiag.reason ? `speech_to_text:${sttDiag.reason}` : "speech_to_text",
    });
  ...
  if (!stt.ok) {
    const errStr = String(stt.error || "");
    const apiLimit = isOpenAiSttApiLimitError(errStr);
    console.error("[whatsapp] audio pipeline: STT provider returned error", {
      ...logBase,
      error: stt.error,
      providerId: stt.providerId ?? null,
      discardReason: apiLimit ? "stt_openai_api_limit" : "stt_provider_failed",
    });
    await trackChatbotEvent({
      sessionId,
      locale: "es",
      ...
      command: "whatsapp_audio",
      mode: "channel",
      outcome: apiLimit ? "stt_openai_quota_or_rate_limit" : "stt_failed",
      details: errStr.slice(0, 220),
    });
    return {
      ok: false,
      reason: apiLimit ? "transcription_api_limit" : "transcription",
      error: stt.error,
    };
  }
