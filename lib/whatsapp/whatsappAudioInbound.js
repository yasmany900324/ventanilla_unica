...
export async function processWhatsAppInboundAudioToText(params) {
  ...
  if (!download || !download.ok) {
    console.error("[whatsapp] audio pipeline: download failed", { ...logBase, error: download.error });
    await trackChatbotEvent({
      sessionId,
      locale: "es",
      userId: null,
  ...
  const normalizedText = normalizeTranscriptionText(stt.text);
  if (!normalizedText) {
    console.warn("[whatsapp] audio pipeline: empty transcript", logBase);
    await trackChatbotEvent({
      sessionId,
      locale: "es",
      userId: null,
