 ...
 *   | { type: 'image'; mediaId: string; mimeType?: string; caption?: string; sourceType?: 'document' }
 *   | { type: 'audio'; mediaId: string; mimeType?: string; voiceNote?: boolean }
...
export function normalizeCloudApiInboundMessage(msg) {
  ...
  // WhatsApp clients can send photos as `document` attachments.
  // Treat image/* documents as image input so the photo step can advance.
  if (type === "document") {
    const document = msg.document && typeof msg.document === "object" ? msg.document : {};
    const mediaId = typeof document.id === "string" ? document.id.trim() : "";
    if (!mediaId) {
      return { type: "unknown", rawType: "document_missing_id" };
    }
    const mimeType =
      typeof document.mime_type === "string" ? document.mime_type.trim().toLowerCase() : "";
    if (!mimeType.startsWith("image/")) {
      return { type: "unknown", rawType: "document_non_image" };
    }
    const caption =
      typeof document.caption === "string" ? document.caption.trim().slice(0, 500) : undefined;
    return {
      type: "image",
      mediaId,
      mimeType,
  ...
  if (type === "audio") {
    ...
    return { type: "audio", mediaId, mimeType, voiceNote: false };
  }

  /** Notas de voz (PTT): Meta usa `voice`, no `audio`; mismo tratamiento para descarga/STT. */
  if (type === "voice") {
    const voice = msg.voice && typeof msg.voice === "object" ? msg.voice : {};
    const mediaId = typeof voice.id === "string" ? voice.id.trim() : "";
    if (!mediaId) {
      return { type: "unknown", rawType: "voice_missing_id" };
    }
    const mimeType =
      typeof voice.mime_type === "string" ? voice.mime_type.trim().toLowerCase() : undefined;
    return { type: "audio", mediaId, mimeType, voiceNote: true };
  }
      ...
      messages.forEach((msg) => {
        ...
        const timestamp =
          tsRaw !== undefined && tsRaw !== null
            ? String(tsRaw).trim().slice(0, 24) || null
            : null;
