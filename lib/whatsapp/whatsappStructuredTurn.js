...
export async function handleWhatsAppStructuredIncidentTurn({
  ...
  if (normalized.type === "image" && isProcedureFlowActive(snapshot)) {
    ...
    const currentField = getProcedureFieldDefinition(requiredFields, currentStepKey);
    const missingBefore = getProcedureMissingFieldsFromDefinition(requiredFields, procedureData);
    const firstMissingField = getProcedureFieldDefinition(requiredFields, missingBefore[0] || "");
    const targetField =
