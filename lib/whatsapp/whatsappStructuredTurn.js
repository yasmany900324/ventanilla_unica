...
function buildIncidentConfirmationContext(collectedData) {
  return {
    ...
    procedureCode:
      typeof collectedData?.procedureCode === "string" ? collectedData.procedureCode.trim().toLowerCase() : "",
    procedureName: typeof collectedData?.procedureName === "string" ? collectedData.procedureName.trim() : "",
    promptType: "final_summary_confirm",
    lastAssistantPrompt: "¿Confirmás que registre esta incidencia con estos datos?",
    collectedDataSnapshot: collectedData && typeof collectedData === "object" ? collectedData : {},
    shownAt: new Date().toISOString(),
  };
}

function buildProcedureConfirmationContext(collectedData) {
  const normalized = normalizeProcedureCollectedData(collectedData || {});
  return {
    flowKey: "procedure.general_start",
    procedureCode:
      typeof normalized.procedureCode === "string" ? normalized.procedureCode.trim().toLowerCase() : "",
    procedureName: typeof normalized.procedureName === "string" ? normalized.procedureName.trim() : "",
    promptType: "final_summary_confirm",
    lastAssistantPrompt: "¿Confirmás que registre esta incidencia con estos datos?",
    collectedDataSnapshot: normalized,
    shownAt: new Date().toISOString(),
  };
}
...
function buildProcedureFieldPrompt(fieldDefinition) {
  if (!fieldDefinition || typeof fieldDefinition !== "object") {
    return "Contame el siguiente dato para continuar.";
  }
  const explicitPrompt = typeof fieldDefinition.prompt === "string" ? fieldDefinition.prompt.trim() : "";
  if (explicitPrompt) {
    return explicitPrompt;
  }
  const label = typeof fieldDefinition.label === "string" ? fieldDefinition.label.trim() : "";
...
export async function handleWhatsAppStructuredProcedureTurn({
  ...
  if (normalized.type === "location" && isProcedureFlowActive(snapshot)) {
    const procedureData = normalizeProcedureCollectedData(snapshot.collectedData || {});
    const fieldDefinitions = Array.isArray(procedureData.procedureFieldDefinitions)
      ? procedureData.procedureFieldDefinitions
      : Array.isArray(procedureData.procedureRequiredFields)
    ...
    const currentStepKey =
      typeof snapshot.currentStep === "string" ? snapshot.currentStep.trim().toLowerCase() : "";
    const currentProcedureField = getProcedureFieldDefinition(fieldDefinitions, currentStepKey);
    const missingBefore = getProcedureMissingFieldsFromDefinition(fieldDefinitions, procedureData);
    const firstMissingField = getProcedureFieldDefinition(fieldDefinitions, missingBefore[0] || "");
    const targetField =
      currentProcedureField?.type === "location"
        ? currentProcedureField
        : firstMissingField?.type === "location"
          ? firstMissingField
          : null;

    if (!targetField || targetField.type !== "location") {
      await track({
        eventName: CHATBOT_TELEMETRY_EVENTS.ENTITIES_REJECTED,
        mode: "procedure",
        outcome: "whatsapp_location_wrong_step_procedure",
        details: `expected=${currentProcedureField?.key || snapshot.currentStep} got=location`,
      });
      return {
        status: 200,
        body: {
          sessionId,
          locale,
          replyText: `Ahora no estamos pidiendo ubicación en este trámite.\n\n${buildProcedureFieldPrompt(
            currentProcedureField
          )}`,
          intent: snapshot.lastIntent || null,
          confidence: snapshot.lastConfidence || null,
          fulfillmentMessages: [],
          action: "whatsapp_location_wrong_step_procedure",
          parameters: {},
          mode: "procedure",
          draft: { ...procedureData, missingFields: missingBefore },
          nextStep: { type: "ask_field", field: snapshot.currentStep },
    ...
    const locationValue = {
      ...
      address: (normalized.addressText || "").slice(0, 400),
      label: (normalized.addressText || "").slice(0, 400),
      locationSource: "whatsapp_location",
    };
    const nextCollected = normalizeProcedureCollectedData({
      ...procedureData,
      location: locationValue,
      [targetField.key]: locationValue,
    ...
    const missingAfter = getProcedureMissingFieldsFromDefinition(fieldDefinitions, nextCollected);
    const nextMissingField = missingAfter[0] || null;
    const nextStep = nextMissingField ? nextMissingField : CHATBOT_CURRENT_STEPS.CONFIRMATION;
    const saved = await setConversationState(
      ...
      createProcedureFlowSnapshotPatch({
        ...
        confirmationContext: !nextMissingField ? buildProcedureConfirmationContext(nextCollected) : null,
        lastInterpretation: {
          ...(snapshot.lastInterpretation || {}),
          whatsappInbound: { type: "location", persisted: true },
        },
        lastIntent: snapshot.lastIntent || "start_procedure",
        lastAction: "whatsapp_location_accepted_procedure",
    ...
    if (!nextMissingField) {
      return {
        ...
        body: {
          ...
          fulfillmentMessages: [],
          action: "whatsapp_location_accepted_procedure",
          parameters: {},
          mode: "procedure",
          draft: { ...nextCollected, missingFields: [] },
          nextStep: { type: "procedure_confirm", field: null },
          actionOptions: [],
          redirectTo: null,
          redirectLabel: null,
          needsClarification: false,
    ...
    const nextFieldDefinition = getProcedureFieldDefinition(fieldDefinitions, nextMissingField);
    return {
      status: 200,
      body: {
        sessionId,
        ...
        parameters: {},
        mode: "procedure",
        draft: { ...nextCollected, missingFields: missingAfter },
        nextStep: { type: "ask_field", field: nextMissingField },
        actionOptions: [],
        redirectTo: null,
        redirectLabel: null,
        needsClarification: false,
  ...
  if (normalized.type === "location" && currentField?.type === "location") {
    ...
    const saved = await setConversationState(
      ...
      createIncidentFlowSnapshotPatch({
        ...
        confirmationContext:
          nextStep === CHATBOT_CURRENT_STEPS.CONFIRMATION
            ? buildIncidentConfirmationContext(nextCollected)
            : null,
        lastInterpretation: {
          ...(snapshot.lastInterpretation || {}),
          whatsappInbound: { type: "location", persisted: true },
  ...
  if (normalized.type === "image" && currentField?.type === "image") {
    ...
    const savedPhoto = await setConversationState(
      ...
      createIncidentFlowSnapshotPatch({
        ...
        confirmationContext:
          nextStep === CHATBOT_CURRENT_STEPS.CONFIRMATION
            ? buildIncidentConfirmationContext(patchedCollected)
            : null,
        lastInterpretation: {
