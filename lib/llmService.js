  ...
  .object({
    ...
    conversationStart: z
      .object({
        ...
        extractedData: z
          .object({
            caseKind: z.enum(["incident", "procedure"]).nullable().default(null),
            procedureHint: z.string().trim().min(1).max(160).nullable().default(null),
            caseIdentifier: z.string().trim().min(1).max(80).nullable().default(null),
          })
          .strict()
          .default({
            caseKind: null,
        ...
        userMessage: z.string().trim().min(1).max(240).nullable().default(null),
      })
      .strict()
      .default({
        intent: "ambiguous",
      ...
      .object({
        ...
        extractedData: z
          .object({
            caseIdentifier: z.string().trim().min(1).max(80).nullable().default(null),
            procedureHint: z.string().trim().min(1).max(160).nullable().default(null),
            caseKind: z.enum(["incident", "procedure", "unknown"]).nullable().default(null),
          })
          .strict()
          .default({
            caseIdentifier: null,
...
- conversationStart.intent solo puede ser: greeting_or_start, start_case, check_status, unsupported, ambiguous.
- conversationStart se usa SOLO para decidir el arranque de una conversación cuando no hay flujo activo.
- conversationStart.extractedData.caseKind solo puede ser: incident, procedure o null.
- conversationStart.extractedData.procedureHint debe ser una pista breve del trámite si aplica.
- conversationStart.extractedData.caseIdentifier debe incluir el identificador detectado para consultas de estado (si existe).
- conversationStart.userMessage puede ser null o un texto breve para pedir aclaración.
- statusFollowUp se usa SOLO cuando sessionContext.statusFlowStage = "status_result_shown".
- statusFollowUp.intent solo puede ser:
  lookup_another_case, start_new_case, provide_case_code, greeting_or_start, ambiguous, unsupported.
- Si sessionContext.statusFlowStage = "status_result_shown" y el usuario expresa que quiere crear/reportar/iniciar un nuevo caso
  (por ejemplo "quiero crear una nueva incidencia", "una nueva incidencia", "quiero reportar un problema"),
  clasifica como statusFollowUp.intent = "start_new_case" con confianza alta cuando la intención sea clara.
- statusFollowUp.extractedData.caseIdentifier debe traer el identificador detectado cuando aplique.
- statusFollowUp.extractedData.caseKind solo puede ser: incident, procedure, unknown o null.
- statusFollowUp.userMessage puede ser null o un texto breve para aclarar.
...
function normalizeRawInterpretation(value) {
  ...
  const conversationStart =
    value.conversationStart && typeof value.conversationStart === "object"
      ? value.conversationStart
      : {};
  const conversationStartExtractedData =
    conversationStart.extractedData && typeof conversationStart.extractedData === "object"
      ? conversationStart.extractedData
      : {};
  const statusFollowUp =
    value.statusFollowUp && typeof value.statusFollowUp === "object" ? value.statusFollowUp : {};
  const statusFollowUpExtractedData =
    statusFollowUp.extractedData && typeof statusFollowUp.extractedData === "object"
      ? statusFollowUp.extractedData
      : {};
  ...
  return {
    ...
    conversationStart: {
      intent: conversationStart.intent,
      confidence: conversationStart.confidence,
      extractedData: {
        caseKind: conversationStartExtractedData.caseKind,
        procedureHint: conversationStartExtractedData.procedureHint,
        caseIdentifier: conversationStartExtractedData.caseIdentifier,
      },
      userMessage: conversationStart.userMessage,
    },
    statusFollowUp: {
      intent: statusFollowUp.intent,
      confidence: statusFollowUp.confidence,
      extractedData: {
        caseIdentifier: statusFollowUpExtractedData.caseIdentifier,
        procedureHint: statusFollowUpExtractedData.procedureHint,
        caseKind: statusFollowUpExtractedData.caseKind,
      },
      userMessage: statusFollowUp.userMessage,
    },
  };
}
