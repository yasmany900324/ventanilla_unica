...
function normalizeFileLikeValue(rawValue) {
  ...
  if (typeof rawValue === "string") {
    ...
    if (isValidImageReference(text)) {
      return { filename: text, originalName: text };
    }
    return null;
  }
  if (typeof rawValue !== "object") {
...
function buildConfigurableFieldAttemptedSources(mapping) {
  ...
  if (t === "image" || t === "file" || t === "attachment") {
    out.push(
      "legacy.photoAttachmentPublicUrl",
      "legacy.photoAttachmentOriginalName",
      "legacy.photoAttachmentStoredFilename",
      "legacy.photoAttachmentStorageKey"
    );
  }
  if (t === "location") {
    out.push(
