...
describe("getCamundaUserTaskForm", () => {
  it("devuelve status ok con schema", async () => {
    process.env.CAMUNDA_REST_ADDRESS = "https://gw.example/v2";
    process.env.CAMUNDA_CLIENT_ID = "client";
    process.env.CAMUNDA_CLIENT_SECRET = "secret";
    process.env.CAMUNDA_OAUTH_URL = "https://login.example/oauth/token";
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ...
        text: async () =>
          JSON.stringify({
            id: "form-1",
            formKey: "Formulario A",
            version: 3,
            schema: { components: [{ type: "textfield", key: "comentario", label: "Comentario" }] },
          }),
      });
    vi.stubGlobal("fetch", fetchMock);
    const out = await getCamundaUserTaskForm("2251799813689001");
    expect(out.status).toBe("ok");
    expect(out.form?.id).toBe("form-1");
    expect(out.form?.key).toBe("Formulario A");
    expect(out.form?.schema?.components?.length).toBe(1);
  });

  it("devuelve status no_form en 404", async () => {
    process.env.CAMUNDA_REST_ADDRESS = "https://gw.example/v2";
    process.env.CAMUNDA_CLIENT_ID = "client";
    process.env.CAMUNDA_CLIENT_SECRET = "secret";
    process.env.CAMUNDA_OAUTH_URL = "https://login.example/oauth/token";
    ...
    vi.stubGlobal("fetch", fetchMock);
    const out = await getCamundaUserTaskForm("2251799813689001");
    expect(out.status).toBe("no_form");
