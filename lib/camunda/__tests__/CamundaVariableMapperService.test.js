...
vi.mock("../../procedureCatalog", async (importOriginal) => {
...
describe("CamundaVariableMapperService", () => {
  ...
  it("normaliza imagen cuando el ciudadano envió solo URL https como texto", async () => {
    mocks.listProcedureTypeCamundaVariableMappings.mockResolvedValue([]);
    mocks.getProcedureCatalogEntryById.mockResolvedValue({
      id: "proc-1",
      code: "tramite_a",
      requiredFields: [{ key: "photo", type: "image", required: true }],
    });
    ...
    const result = await service.buildVariables({
      ...
      includeProcedureFieldDefinitions: true,
      collectedData: {
        photo: "https://example.org/files/photo.jpg",
      },
    });
    expect(result).toEqual({
      photo: { url: "https://example.org/files/photo.jpg" },
    });
  });

  it("acepta imagen cuando solo existe filename (sin URL pública)", async () => {
    mocks.listProcedureTypeCamundaVariableMappings.mockResolvedValue([]);
    mocks.getProcedureCatalogEntryById.mockResolvedValue({
      id: "proc-1",
      code: "tramite_a",
