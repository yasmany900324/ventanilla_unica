...
describe("syncLocalCaseToCamunda", () => {
  ...
  it("si el process id del catalogo devuelve 404, reintenta con fallback env", async () => {
    mocks.getActiveCatalogItemByCode.mockResolvedValue({
      id: "proc-catalog-2",
      caseType: "procedure",
    ...
    notFoundError.detail = {
      detail:
        "Expected to find process definition with process ID 'incidencias', but none found",
    };
    mocks.createCamundaProcessInstance
      .mockRejectedValueOnce(notFoundError)
      .mockResolvedValueOnce({
        processInstanceKey: "777",
        processDefinitionId: "fallback_tramite",
      });

    await syncTramiteToCamundaAfterCreate({
      id: "tra-404",
      procedureCode: "registrar_incidencia",
      requestCode: "TRA-404TEST",
      summary: "sum",
      status: "recibido",
    });

    expect(mocks.createCamundaProcessInstance).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ processId: "incidencias" })
    );
    expect(mocks.createCamundaProcessInstance).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ processId: "fallback_tramite" })
    );
    expect(mocks.upsertCamundaCaseLink).toHaveBeenCalledWith(
      expect.objectContaining({
        localCaseId: "tra-404",
        camundaState: "started",
        camundaProcessDefinitionId: "fallback_tramite",
      })
    );
  });
