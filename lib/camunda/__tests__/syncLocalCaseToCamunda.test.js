...
describe("syncLocalCaseToCamunda", () => {
  ...
  it("no crea nueva instancia si ya existe camunda_case_link con instance key", async () => {
    mocks.getCamundaCaseLinkByLocalCase.mockResolvedValue({
      camundaProcessInstanceKey: "linked-123",
      camundaProcessDefinitionId: "Process_1hvmc45",
    });
    mocks.getActiveCatalogItemByCode.mockResolvedValue({
      id: "proc-catalog-1",
    ...
    const out = await syncTramiteToCamundaAfterCreate({
      id: "tra-linked",
      procedureCode: "habilitacion_comercial",
      requestCode: "TRA-LINKED",
      summary: "sum",
      status: "PENDING_CAMUNDA_SYNC",
    });

    expect(out.ok).toBe(true);
    expect(out.alreadySynced).toBe(true);
    expect(out.processInstanceKey).toBe("linked-123");
    expect(mocks.createCamundaProcessInstance).not.toHaveBeenCalled();
  });
  ...
  it("reintento no explota si falla la métrica de intentos", async () => {
    mocks.incrementProcedureMetric.mockRejectedValueOnce(new Error("metric down"));
    mocks.getProcedureRequestById.mockResolvedValueOnce({
      id: "pr-retry-1",
      procedureTypeId: "proc-catalog-1",
      procedureCode: "habilitacion_comercial",
    ...
    mocks.getActiveCatalogItemByCode.mockResolvedValueOnce({
      id: "proc-catalog-1",
      caseType: "procedure",
      camundaProcessId: "Process_1hvmc45",
    });
