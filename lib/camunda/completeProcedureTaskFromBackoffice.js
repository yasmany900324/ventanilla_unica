...
export async function completeProcedureTaskFromBackoffice({
  ...
  const previousTaskId = String(activeTask.taskId || "").trim() || null;
  ...
  const pollingProcedureRequest = {
    ...procedureRequest,
    camundaProcessInstanceKey: processInstanceKey,
  };
  const waitResult = await waitForCamundaSnapshotChange({
    procedureRequest: pollingProcedureRequest,
    actorId,
    action: "complete_task",
    predicate: (snapshot) => {
      const processState = String(snapshot?.process?.state || "").trim().toUpperCase();
      if (processState === "COMPLETED") {
        return true;
      }
      const hasTask = snapshot?.activeTask?.exists === true;
      if (!hasTask) {
        return true;
      }
      const nextTaskId = String(snapshot?.activeTask?.id || snapshot?.activeTask?.taskId || "").trim() || null;
      const nextTaskDefinitionKey = String(snapshot?.activeTask?.taskDefinitionKey || "").trim() || null;
      if (previousTaskId && nextTaskId && nextTaskId !== previousTaskId) {
        return true;
      }
      if (taskDefinitionKey && nextTaskDefinitionKey && nextTaskDefinitionKey !== taskDefinitionKey) {
        return true;
      }
      return false;
    },
  ...
  return {
    ...
    camundaAction: "complete_task",
    syncStatus: waitResult?.confirmed ? "confirmed" : "pending",
    previousTaskId,
    snapshot,
    camunda: {
      processInstanceKey,
