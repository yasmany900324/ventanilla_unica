 ...
 * @param {'started'|'camunda_sync_failed'|'camunda_sync_skipped'} params.camundaState
 * @param {string|null} [params.camundaProcessInstanceKey]
 * @param {string|null} [params.catalogItemId]
...
export async function getCamundaCaseLinkByLocalCase({ localCaseId, localCaseType }) {
  if (!hasDatabase()) {
    return null;
  }
  const normalizedCaseId = typeof localCaseId === "string" ? localCaseId.trim() : "";
  const normalizedCaseType = typeof localCaseType === "string" ? localCaseType.trim() : "";
  if (!normalizedCaseId || !normalizedCaseType) {
    return null;
  }
  const sql = ensureDatabase();
  await ensureCamundaCaseLinkSchema();
  const [row] = await sql`
    SELECT
      ...
      created_at,
      updated_at
    FROM camunda_case_links
    WHERE local_case_id = ${normalizedCaseId}
      AND local_case_type = ${normalizedCaseType}
    LIMIT 1;
  `;
  if (!row) {
    return null;
  ...
  return {
    ...
    localCaseType: row.local_case_type,
    ...
    camundaState: row.camunda_state,
    startedAt: row.started_at || null,
    rawResponse: row.raw_response_json && typeof row.raw_response_json === "object" ? row.raw_response_json : {},
