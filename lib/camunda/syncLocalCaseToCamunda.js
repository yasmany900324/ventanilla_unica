...
import { updateProcedureRequestCamundaData } from "../procedureRequests";
import {
...
export async function syncIncidentToCamundaAfterCreate(incident, context = {}) {
  ...
  try {
    ...
    await updateProcedureRequestCamundaData({
      ...
      camundaProcessInstanceKey: instanceKey,
      status: procedureRequest?.status || "recibido",
    });
  } catch (error) {
...
export async function syncTramiteToCamundaAfterCreate(procedureRequest, context = {}) {
  ...
  let effectiveSource = processId ? "procedure_catalog" : "procedure_catalog_missing_process_id";
  ...
  try {
    ...
    if (!variables || Object.keys(variables).length === 0) {
      variables = buildTramiteCamundaVariables(procedureRequest, context);
    }
