...
import { getCamundaCaseLinkByLocalCase, upsertCamundaCaseLink } from "./camundaCaseLinks";
...
export async function syncTramiteToCamundaAfterCreate(procedureRequest, context = {}) {
  ...
  const existingLink = await getCamundaCaseLinkByLocalCase({
    localCaseId: persistedProcedure.id,
    localCaseType: "tramite",
  }).catch(() => null);
  const linkedInstanceKey = String(existingLink?.camundaProcessInstanceKey || "").trim();
  if (linkedInstanceKey) {
    await updateProcedureRequestCamundaData({
      procedureRequestId: persistedProcedure.id,
      camundaProcessInstanceKey: linkedInstanceKey,
      camundaProcessDefinitionId:
        String(existingLink?.camundaProcessDefinitionId || "").trim() || undefined,
      clearCamundaError: true,
    });
    return {
      ok: true,
      alreadySynced: true,
      processInstanceKey: linkedInstanceKey,
      source: "existing_camunda_case_link",
    };
  }
  const catalogProcess = await resolveCatalogProcessId({
    caseType: "procedure",
  ...
  try {
    ...
    const activeTask = await getActiveTaskForProcedure(persistedProcedure.id).catch(async () => {
      for (const state of ["CREATED", "ASSIGNED"]) {
        ...
        if (firstTask) {
          return { taskDefinitionKey: pickTaskDefinitionKey(firstTask) || null };
        }
      }
      return null;
    });
