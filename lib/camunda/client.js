...
/**
 * Obtiene el BPMN XML de una definición de proceso (Orchestration REST GET /process-definitions/{key}/xml).
 * Respuesta eventualmente consistente en Camunda 8.
 *
 * @param {string} processDefinitionKey Clave o id de definición (p. ej. el `processDefinitionId` de la instancia).
 * @returns {Promise<string>} Contenido XML BPMN
 */
export async function getCamundaProcessDefinitionXml(processDefinitionKey) {
  const base = getCamundaBaseUrl();
  if (!base) {
    throw new CamundaClientError("Camunda: URL REST no configurada.");
  }
  const key = String(processDefinitionKey ?? "").trim();
  if (!key) {
    throw new CamundaClientError("getCamundaProcessDefinitionXml: processDefinitionKey inválido.");
  }

  const token = await getCamundaAccessToken();
  const url = `${base}/process-definitions/${encodeURIComponent(key)}/xml`;
  console.info(`${LOG_PREFIX} Obteniendo BPMN XML de definición`, sanitizeForLogs({ url: urlForLogs(url), keySlice: key.slice(0, 80) }));
  ...
  if (res.status === 401 || res.status === 403) {
    console.warn(
      `${LOG_PREFIX} BPMN XML rechazado por permisos o token`,
      sanitizeForLogs({ status: res.status, url: urlForLogs(url) })
    );
    throw new CamundaClientError(
      res.status === 403
        ? "Camunda rechazó el acceso al BPMN XML (permisos insuficientes o definición no visible para el cliente OAuth)."
        : "Camunda rechazó la autenticación al solicitar BPMN XML.",
      { status: res.status, errorCode: "CAMUNDA_BPMN_XML_FORBIDDEN" }
    );
  }

  if (res.status === 404) {
    console.warn(`${LOG_PREFIX} BPMN XML no encontrado (404)`, sanitizeForLogs({ keySlice: key.slice(0, 80) }));
    throw new CamundaClientError("Definición de proceso no encontrada para obtener BPMN XML.", {
      status: 404,
      errorCode: "CAMUNDA_BPMN_XML_NOT_FOUND",
    });
  }

  if (res.status === 204) {
    console.warn(`${LOG_PREFIX} BPMN XML vacío (204)`, sanitizeForLogs({ keySlice: key.slice(0, 80) }));
    throw new CamundaClientError("Camunda devolvió BPMN XML vacío (204).", {
      status: 204,
      errorCode: "CAMUNDA_BPMN_XML_EMPTY",
    });
  }
  ...
  if (!res.ok) {
    ...
    console.warn(
      `${LOG_PREFIX} Fallo al obtener BPMN XML`,
      sanitizeForLogs({ status: res.status, bodySlice: rawText?.slice(0, 400) })
    );
    throw new CamundaClientError(`Camunda get BPMN XML falló (HTTP ${res.status}).`, {
      status: res.status,
      detail: json ?? rawText?.slice(0, 800),
      errorCode: "CAMUNDA_BPMN_XML_ERROR",
    });
  }
  ...
  if (contentType.includes("json")) {
    ...
    const xml =
      json && typeof json === "object"
        ? String(json.xml ?? json.bpmnXml ?? json.bpmnXML ?? json.resource ?? "").trim()
        : "";
    if (xml) {
      return xml;
    }
    console.warn(`${LOG_PREFIX} Respuesta JSON sin campo xml reconocido`, sanitizeForLogs({ keys: json ? Object.keys(json).slice(0, 20) : [] }));
    throw new CamundaClientError("Camunda devolvió JSON sin XML BPMN reconocible.", {
      status: res.status,
      errorCode: "CAMUNDA_BPMN_XML_UNEXPECTED_JSON",
    });
  }

  const trimmed = String(rawText || "").trim();
  if (!trimmed || !trimmed.includes("<")) {
    console.warn(`${LOG_PREFIX} Cuerpo de BPMN XML no parece XML`, sanitizeForLogs({ length: trimmed.length }));
    throw new CamundaClientError("Camunda devolvió un cuerpo vacío o no XML.", {
      status: res.status,
      errorCode: "CAMUNDA_BPMN_XML_INVALID_BODY",
    });
  }
...
/**
 * Busca definiciones de proceso por BPMN process id (Orchestration REST POST /process-definitions/search).
 * @param {{ bpmnProcessId: string, pageSize?: number }} params
 * @returns {Promise<Array<Record<string, unknown>>>}
 */
export async function searchCamundaProcessDefinitions({ bpmnProcessId, pageSize = 5 } = {}) {
  const base = getCamundaBaseUrl();
  if (!base) {
    throw new CamundaClientError("Camunda: URL REST no configurada.");
  }
  const normalizedBpmnProcessId = String(bpmnProcessId || "").trim();
  if (!normalizedBpmnProcessId) {
    throw new CamundaClientError("searchCamundaProcessDefinitions: bpmnProcessId inválido.");
  }
  const limit = Number.isInteger(pageSize) && pageSize > 0 ? Math.min(pageSize, 20) : 5;
  const token = await getCamundaAccessToken();
  const url = `${base}/process-definitions/search`;
  const payload = {
    filter: {
      processDefinitionId: normalizedBpmnProcessId,
    },
    page: {
      limit,
    },
  ...
  if (!res.ok) {
    throw new CamundaClientError(`Camunda search process definitions falló (HTTP ${res.status}).`, {
      status: res.status,
      detail: json ?? rawText?.slice(0, 800),
      errorCode: "CAMUNDA_PROCESS_DEFINITION_SEARCH_ERROR",
    });
  }
...
/**
 * Obtiene el formulario asociado a una user task activa.
 * Endpoint: GET /v2/user-tasks/{userTaskKey}/form
 *
 * @param {string|number} userTaskKey
 * @returns {Promise<{
 *   status: "ok"|"no_form"|"error",
 *   httpStatus: number,
 *   form: { id: string|null, key: string|null, version: number|null, schema: Record<string, unknown>|null }|null,
 *   message: string|null,
 * }>}
 */
export async function getCamundaUserTaskForm(userTaskKey) {
  const base = getCamundaBaseUrl();
  if (!base) {
    throw new CamundaClientError("Camunda: URL REST no configurada.");
  }
  const normalizedUserTaskKey = String(userTaskKey || "").trim();
  if (!normalizedUserTaskKey) {
    throw new CamundaClientError("getCamundaUserTaskForm: userTaskKey inválido.");
  }
  const token = await getCamundaAccessToken();
  const url = `${base}/user-tasks/${encodeURIComponent(normalizedUserTaskKey)}/form`;
  const res = await fetch(url, {
    method: "GET",
    headers: {
      Accept: "application/json",
  ...
  if (res.status === 204 || res.status === 404) {
    console.info(
      `${LOG_PREFIX} user task form consultado`,
      sanitizeForLogs({
        userTaskKey: normalizedUserTaskKey,
        status: res.status,
        found: false,
      })
    );
    return {
      ...
      httpStatus: res.status,
      form: null,
      message: "La tarea activa no tiene formulario asociado.",
    };
  }
  if (res.status === 401 || res.status === 403) {
    console.warn(
      `${LOG_PREFIX} user task form rechazado`,
      sanitizeForLogs({
        userTaskKey: normalizedUserTaskKey,
        status: res.status,
        found: false,
    ...
    return {
      ...
      httpStatus: res.status,
      form: null,
      message: "Camunda rechazó el acceso al formulario de la tarea.",
    };
  }
  if (!res.ok) {
    console.error(
      `${LOG_PREFIX} user task form falló`,
      sanitizeForLogs({
        userTaskKey: normalizedUserTaskKey,
        status: res.status,
        body: json ?? rawText?.slice(0, 800),
      })
    );
    return {
      status: "error",
      httpStatus: res.status,
      form: null,
      message: "No se pudo obtener el formulario de la tarea en Camunda.",
    };
  }

  const rawSchema = json?.schema ?? null;
  let schema = null;
  ...
  const form = {
    id: json?.id != null && String(json.id).trim() ? String(json.id).trim() : null,
    key: json?.formKey != null && String(json.formKey).trim() ? String(json.formKey).trim() : null,
    version: Number.isFinite(Number(json?.version)) ? Number(json.version) : null,
    schema,
  };
  console.info(
    `${LOG_PREFIX} user task form consultado`,
