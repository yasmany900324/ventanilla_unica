...
export async function searchCamundaUserTasks({
  ...
  // Camunda Orchestration REST v2 no expone ASSIGNED; la asignación se infiere por `assignee`.
  ...
  if (!res.ok) {
    ...
    console.error(
      ...
      sanitizeForLogs({
        ...
        processInstanceKeyArg: processInstanceKey,
        processInstanceKeyArgTypeof: typeof processInstanceKey,
        processInstanceKeyInFilter: keyTrimmed || null,
        stateRequested: state,
        stateRequestedNormalized: requestedState,
        stateRemappedForApi: requestedState === stateNormalized ? null : `${requestedState}->${stateNormalized}`,
        stateFormat: "UPPERCASE_STRING_IN_FILTER",
        stateSent: stateNormalized,
