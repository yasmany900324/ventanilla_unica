...
export async function getLiveCamundaTaskSnapshot({ procedureRequest, actorId = null } = {}) {
  ...
  try {
    ...
    const mappedCreated = (createdTasks || []).map(mapCamundaOrchestrationUserTask).filter(Boolean);
    ...
    const output = {
      ...
      process: {
        ...
        bpmnProcessId: String(processInstance.bpmnProcessId || "").trim() || null,
        definitionId: String(processInstance.processDefinitionId || "").trim() || null,
        definitionKey:
          String(processInstance.processDefinitionKey || processInstance.definitionKey || "").trim() || null,
      },
    ...
    if (isActiveProcess && !output.activeTask.exists) {
      ...
      if (createdTasks.length > 0 && mappedCreated.length === 0) {
        console.info(
          "[camunda] user-tasks/search items descartados por mapTask (sin userTaskKey+elementId ni id+taskDefinitionKey)",
          sanitizeForLogs({
            procedureRequestId: localCaseId || null,
            processInstanceKey,
            rawFirstItemKeys:
