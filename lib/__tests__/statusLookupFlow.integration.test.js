...
describe("status lookup flow (web + whatsapp)", () => {
  ...
  it("whatsapp: en status_result_shown, 'quiero crear una nueva incidencia' con start_new_case inicia flujo", async () => {
    ...
    const out = await processAssistantTurn({
      channel: "whatsapp",
      sessionId,
      text: "quiero crear una nueva incidencia",
      whatsappWaId: waId,
    });

    expect(llm.interpretUserMessage).toHaveBeenCalledTimes(2);
    expect(String(out.body.replyText || "")).not.toContain("Puedo ayudarte a consultar otro caso");
    expect(out.snapshot?.flowKey).not.toBe("status.check");
  });

  it("whatsapp: en status_result_shown, 'una nueva incidencia' con start_new_case inicia flujo", async () => {
    const waId = "598991239990";
    const sessionId = buildWhatsAppAssistantSessionId(waId);
    llm.interpretUserMessage.mockResolvedValueOnce({
      interpretation: buildStartInterpretation({ intent: "greeting_or_start" }),
      meta: { source: "llm", reason: null },
    ...
    await processAssistantTurn({ channel: "whatsapp", sessionId, text: "2", whatsappWaId: waId });
    incidents.findIncidentByIdentifier.mockResolvedValueOnce({
      id: "ebbece0d-0000-4000-8000-000000000016",
      status: "en revision",
      category: "incidencia_general",
      location: "Centro",
      description: "Caso previo",
  ...
  it("whatsapp: en status_result_shown, start_new_case con un único procedimiento lo autoselecciona", async () => {
    const waId = "598991239989";
    const sessionId = buildWhatsAppAssistantSessionId(waId);
    llm.interpretUserMessage.mockResolvedValueOnce({
      interpretation: buildStartInterpretation({ intent: "greeting_or_start" }),
      meta: { source: "llm", reason: null },
    ...
    await processAssistantTurn({ channel: "whatsapp", sessionId, text: "2", whatsappWaId: waId });
    incidents.findIncidentByIdentifier.mockResolvedValueOnce({
      id: "ebbece0d-0000-4000-8000-000000000017",
      status: "recibido",
      category: "incidencia_general",
      location: "Centro",
      description: "Caso previo",
    ...
    procedureCatalog.listActiveProcedureCatalog.mockResolvedValueOnce([
      {
        id: "proc-visible",
        code: "reportar_problema_comun",
    ...
    llm.interpretUserMessage
      ...
      .mockResolvedValueOnce({
        interpretation: buildStartInterpretation({ intent: "ambiguous", confidence: 0.82 }),
        meta: { source: "llm", reason: null },
      });
    const out = await processAssistantTurn({
      channel: "whatsapp",
    ...
    expect(String(out.body.replyText || "")).toContain('Entendido. Voy a ayudarte con "Reportar un problema común".');
    expect(out.body.nextStep).toEqual({ type: "ask_field", field: "description" });
  });

  it("whatsapp: en status_result_shown, start_new_case con varios procedimientos muestra catálogo", async () => {
    const waId = "598991239988";
    const sessionId = buildWhatsAppAssistantSessionId(waId);
    llm.interpretUserMessage.mockResolvedValueOnce({
      interpretation: buildStartInterpretation({ intent: "greeting_or_start" }),
      meta: { source: "llm", reason: null },
    ...
    await processAssistantTurn({ channel: "whatsapp", sessionId, text: "2", whatsappWaId: waId });
    incidents.findIncidentByIdentifier.mockResolvedValueOnce({
      id: "ebbece0d-0000-4000-8000-000000000018",
      status: "recibido",
      category: "incidencia_general",
      location: "Centro",
      description: "Caso previo",
    ...
    expect(reply).toContain("reportar problema");
  ...
  it("whatsapp: tras aclaración ambigua, una intención clara posterior se procesa", async () => {
    const waId = "598991239987";
    const sessionId = buildWhatsAppAssistantSessionId(waId);
    llm.interpretUserMessage.mockResolvedValueOnce({
      interpretation: buildStartInterpretation({ intent: "greeting_or_start" }),
      meta: { source: "llm", reason: null },
    ...
    await processAssistantTurn({ channel: "whatsapp", sessionId, text: "2", whatsappWaId: waId });
    incidents.findIncidentByIdentifier.mockResolvedValueOnce({
      id: "ebbece0d-0000-4000-8000-000000000019",
      status: "en proceso",
      category: "incidencia_general",
      location: "Centro",
      description: "Caso previo",
    ...
    expect(String(first.body.replyText || "").toLowerCase()).toContain("¿qué querés hacer?");
    ...
    expect(String(second.body.replyText || "")).not.toContain("Puedo ayudarte a consultar otro caso");
    expect(second.snapshot?.flowKey).not.toBe("status.check");
  });
