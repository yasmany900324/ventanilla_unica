...
const authenticatedUser = { id: "citizen-integration-1" };
...
describe("Flujo WhatsApp (integración, sin DB de incidencias)", () => {
  ...
  it("ubicación WhatsApp + sin foto + confirmación llama createIncident con lat/lng", async () => {
    const sessionId = `wa-int-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  ...
  it("imagen WhatsApp en paso foto dispara descarga, persistencia y createIncident con adjunto", async () => {
    const sessionId = `wa-img-${Date.now()}-${Math.random().toString(36).slice(2)}`;
