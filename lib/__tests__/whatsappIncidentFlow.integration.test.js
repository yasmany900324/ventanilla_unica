...
describe("Flujo WhatsApp (integración, sin DB de incidencias)", () => {
  ...
  it("tras crear incidencia, mensajes nuevos no reutilizan el resumen ni el borrador", async () => {
    const sessionId = buildWhatsAppAssistantSessionId(`${testWaId}3`);

    await processAssistantTurn({
      channel: "whatsapp",
      sessionId,
      text: "quiero reportar una incidencia por basura en la vía",
      authenticatedUser: null,
      whatsappWaId: `${testWaId}3`,
      channelInbound: null,
    });
    ...
    expect(snapAfterCreate?.confirmationState).toBe("none");
    expect(snapAfterCreate?.state).toBe(CHATBOT_CONVERSATION_STATES.CLOSED);
    expect(String(snapAfterCreate?.collectedData?.location || "")).toBe("");

    const afterHola = await processAssistantTurn({
      channel: "whatsapp",
      sessionId,
      text: "Hola",
      authenticatedUser: null,
    ...
    const holaReply = String(afterHola.body?.replyText || "");
    expect(holaReply).not.toContain("Te muestro el resumen");
    expect(holaReply).not.toContain("Resumen del reporte");
