...
describe("Trámite WhatsApp sin usuario portal", () => {
  ...
  it("en modo procedure-only, 'árbol caído' entra a flujo procedure", async () => {
  ...
  it("en modo procedure-only, si no hay procedure compatible responde que no puede ayudar", async () => {
    process.env.CHATBOT_CATALOG_CASE_TYPE = "procedure";
    ...
    expect(out.status).toBe(200);
    expect(String(out.body?.replyText || "").toLowerCase()).toContain(
      "de momento no puedo ayudarte con ese trámite"
    );
