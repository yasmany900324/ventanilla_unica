...
describe("procedureRequests inbox ownership local", () => {
  beforeEach(() => {
    ...
    state.request = {
      id: "pr-1",
      channel: "WHATSAPP",
      whatsapp_wa_id: "5988877766655",
      request_code: "TRA-205217E1",
  ...
  it("expediente creado desde WhatsApp sin owner aparece como disponible en bandeja", async () => {
    ...
    expect(beforeEnriched[0].channel).toBe("WHATSAPP");
    expect(beforeEnriched[0].requestCode).toBe("TRA-205217E1");
    expect(beforeEnriched[0].assignmentScope).toBe("available");
    expect(beforeEnriched[0].pendingAction).toBe("Tomar expediente");
    expect(beforeEnriched[0].assignedToUserId).toBeNull();
  });

  it("si funcionario toma expediente disponible pasa a asignado a mí", async () => {
    ...
    expect(firstRefresh[0].assignedToUserId).toBe("func-1");
