...
describe("procedure field definitions compatibility", () => {
  ...
  it("conserva casing de la key técnica configurable", () => {
    const normalized = normalizeProcedureCollectedData({
      procedureFieldDefinitions: [{ key: "comprobantePago", type: "image", required: true }],
      comprobantePago: { url: "https://example.org/comp.png" },
    });

    expect(normalized.procedureFieldDefinitions[0].key).toBe("comprobantePago");
    expect(normalized.comprobantePago).toEqual({ url: "https://example.org/comp.png" });
    expect(normalized.comprobantepago).toBeUndefined();
  });

  it("copia la foto del flujo de incidencia (photoAttachmentPublicUrl) al campo photo del trámite", () => {
    const normalized = normalizeProcedureCollectedData({
      procedureFieldDefinitions: [
      ...
      photoAttachmentPublicUrl: "https://storage.example.com/draft/abc.jpg",
    });

    expect(normalized.photo).toEqual({
      url: "https://storage.example.com/draft/abc.jpg",
    });
  });

  it("usa metadata legacy de adjunto cuando no hay publicUrl", () => {
    const normalized = normalizeProcedureCollectedData({
      procedureFieldDefinitions: [{ key: "photo", type: "image", required: true }],
      photoAttachmentStoredFilename: "draft-abc.jpg",
      photoAttachmentMimeType: "image/jpeg",
      photoAttachmentSizeBytes: 3210,
    });

    expect(normalized.photo).toEqual({
      filename: "draft-abc.jpg",
      mimeType: "image/jpeg",
      size: 3210,
    });
  });
