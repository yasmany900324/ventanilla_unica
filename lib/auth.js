...
export async function registerCitizen({
  ...
  const [row] = await sql`
    INSERT INTO citizens (id, full_name, cedula, email, password_hash, updated_at)
    VALUES (${userId}, ${normalizedFullName}, ${normalizedCedula}, ${normalizedEmail}, ${passwordHash}, NOW())
    RETURNING id, full_name, cedula, email, created_at, updated_at;
  `;
...
export async function loginCitizen({ identifier, password }) {
  ...
  const [row] = await sql`
    SELECT id, full_name, cedula, email, password_hash, created_at, updated_at
    FROM citizens
    WHERE ${
      isIdentifierEmail
