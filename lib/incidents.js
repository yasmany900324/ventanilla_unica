...
import { getActiveCatalogItemById } from "./procedureCatalog";
  ...
  const attachmentImageUrl =
    typeof attachmentUrl === "string" && attachmentUrl.startsWith("http")
      ...
      : hasAttachment
        ...
        : null;
  const latRaw = row.location_latitude;
  const lngRaw = row.location_longitude;
  const geo = coerceIncidentGeoCoords(latRaw, lngRaw);
  return {
    id: row.id,
    userId: row.user_id,
    ...
    locationLatitude: geo.locationLatitude,
    locationLongitude: geo.locationLongitude,
    status: row.status,
...
const INCIDENT_ROW_SELECT_FIELDS = `
  id,
  user_id,
  whatsapp_wa_id,
  catalog_item_id,
...
/** @deprecated Usar INCIDENT_ROW_SELECT_FIELDS */
const ATTACHMENT_SELECT_FIELDS = INCIDENT_ROW_SELECT_FIELDS;

/**
 * Valida y normaliza coordenadas para persistencia en incidencias.
 * @param {unknown} latitude
 * @param {unknown} longitude
 * @returns {{ locationLatitude: number|null, locationLongitude: number|null }}
 */
export function coerceIncidentGeoCoords(latitude, longitude) {
  ...
  return { locationLatitude: lat, locationLongitude: lng };
}
...
export async function ensureSchema() {
  ...
  await sql`
    CREATE INDEX IF NOT EXISTS incidents_whatsapp_wa_id_created_at_idx
    ON incidents (whatsapp_wa_id, created_at DESC)
...
/**
 * @param {object} params
 * @param {string|null} [params.userId] — ciudadano autenticado (portal web)
 * @param {string|null} [params.whatsappWaId] — identificador WhatsApp (`wa_id` Meta, dígitos); obligatorio si no hay `userId`
 * @param {string} params.category
 * @param {string} params.description
 * @param {string} params.location
 * @param {number|null} [params.locationLatitude]
 * @param {number|null} [params.locationLongitude]
 * @param {null|{
 *   storageProvider?: string,
 *   storageKey?: string,
...
export async function createIncident({
  ...
  description,
  location,
  locationLatitude: locationLatitudeInput = null,
  locationLongitude: locationLongitudeInput = null,
  ...
  if (!hasPortalUser && !waDigits) {
    throw new Error("createIncident: se requiere userId (portal) o whatsappWaId (WhatsApp).");
  }
  if (hasPortalUser && waDigits) {
    throw new Error("createIncident: no mezclar userId de portal con whatsappWaId en el mismo registro.");
  }
  ...
  const normalizedCatalogItemId =
    typeof catalogItemId === "string" && catalogItemId.trim()
      ? catalogItemId.trim().slice(0, 80)
      : null;
  const catalogItem = normalizedCatalogItemId
    ? await getActiveCatalogItemById(normalizedCatalogItemId)
    : null;
  const requestedCategory =
    typeof category === "string" ? category.replace(/\s+/g, " ").trim().slice(0, 80) : "";
  const resolvedCategory =
    requestedCategory ||
    (catalogItem?.caseType === "incident" ? String(catalogItem.category || "").trim().slice(0, 80) : "") ||
    "incidencia_general";
  ...
  try {
    const [row] = await sql`
      ...
      VALUES (
        ...
        ${waDigits},
        ${catalogItem?.caseType === "incident" ? normalizedCatalogItemId : null},
        ${resolvedCategory},
        ${description},
        ${location},
        ${locationLatitude},
      ...
      RETURNING ${sql.unsafe(INCIDENT_ROW_SELECT_FIELDS)};
    `;
...
export async function findIncidentByIdentifier({ userId, whatsappWaId, identifier }) {
  const sql = ensureDatabase();
  await ensureSchema();
