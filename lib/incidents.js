...
export async function listIncidents() {
  ...
  const rows = await sql`
    SELECT id, category, description, location, status, created_at
    FROM incidents
...
export async function createIncident({ category, description, location }) {
  ...
  await ensureSchema();
  const [row] = await sql`
    INSERT INTO incidents (id, category, description, location, status)
    VALUES (${id}, ${category}, ${description}, ${location}, 'recibido')
    RETURNING id, category, description, location, status, created_at;
  `;
